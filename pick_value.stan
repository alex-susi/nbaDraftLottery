...
parameters {
  ...
  real<lower=0.01> beta;           // decay rate across slots
...
model {
  // Weakly-informative priors, scaled to the 4-year WS range.
  // A #1 pick averages ~20-25 WS over 4 yrs; late firsts ~2-4 WS.
  alpha       ~ normal(20, 6);
  beta        ~ normal(0.12, 0.05);
  gamma       ~ normal(2.0, 1.5);
  sigma_base  ~ exponential(0.5);
  sigma_slope ~ exponential(5);

  // Likelihood: observed slot mean ~ Normal(curve, sqrt(se^2 + model^2))
  for (i in 1:N) {
...
generated quantities {
  // Posterior predictive curve + log-lik for diagnostics (LOO/PSIS).
  vector[30] war_pred;
