// VERSION: pick_play_r2_v6_declining_upside.stan
// Right-skew shifted-lognormal mixture with pick-declining rare-upside probability and exact-pick ws4_pick_rep output.
...
// produce star-level outcomes, while its strongly regularized, pick-declining mixture
...
parameters {
  ...
  // Rare-upside component. The probability is pick-specific and forced to
  // decline across the round; the component scale is capped to prevent an
  // explosive far-right lognormal tail.
  real logit_upside_31;
  real<upper=0> delta_logit_upside;
  real<lower=0> upside_log_shift;
  real<lower=1, upper=2.25> upside_sigma_mult;
}
...
transformed parameters {
  ...
  vector[30] logit_upside_pick;
  vector<lower=0, upper=1>[30] p_upside_pick;
  real<lower=0, upper=1> upside_prob_31;
  real<lower=0, upper=1> upside_prob_45;
  real<lower=0, upper=1> upside_prob_60;
  ...
  for (p in 1:30) {
    logit_upside_pick[p] = logit_upside_31 + delta_logit_upside * (p - 1);
  }
  p_upside_pick = inv_logit(logit_upside_pick);
...
model {
  ...
  // Rare upside. The prior identifies this as a true high-end-hit component,
  // not a broad second component for ordinary rotation outcomes. At the prior
  // mean, P(upside | played) is about 10% at pick 31 and about 3%-4% by pick 60.
  logit_upside_31 ~ normal(logit(0.10), 0.35);
  delta_logit_upside ~ normal(-0.04, 0.015);
  upside_log_shift ~ normal(log(3.0), 0.35);
  upside_sigma_mult ~ lognormal(log(1.25), 0.15);
...
generated quantities {
  ...
  for (p in 1:30) {
    ...
    real p_up = p_upside_pick[p];
    real shift_m1 = (1 - p_up) * typ_m1 + p_up * up_m1;
    real shift_m2 = (1 - p_up) * typ_m2 + p_up * up_m2;
    real cond_second;
    real cond_var;
  ...
  for (n in 1:N) {
    ...
    if (played_rep[n] == 1) {
      if (bernoulli_rng(p_upside_pick[s]) == 1) {
