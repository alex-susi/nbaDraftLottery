...
- $\delta$ — additive log-location shift of the upside component (how much larger the rare outcome is in log-median terms).
- $\kappa$ — multiplicative inflation of the upside component's dispersion, capped to prevent an explosive far-right tail.
- $d_\pi,\, d_m$ — deterministic drift terms; $\tau_\pi,\, \tau_m,\, \tau_s$ — random-walk innovation scales; $z^{\pi}_p,\, z^{m}_p,\, z^{s}_p$ — standardized innovations.
