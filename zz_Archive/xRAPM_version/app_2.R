################################################################################
# NBA 3-2-1 Lottery Reform — Shiny Dashboard (v6, TPI + both rounds)
#
# Reads dashboard_data.rds produced by nba_lottery_2.R
#
# v6 changes:
#   * Value unit is first-4-year TOTAL POINTS IMPACT (TPI = xRAPM/100 x
#     possessions, summed over the 4 calendar seasons after the draft) —
#     replaces 4-year Win Shares everywhere. TPI can be NEGATIVE.
#   * Pick curve and asset registry span picks 1-60 (both rounds); the curve
#     plot marks the round boundary at 30.5.
#   * Trade Machine is round-aware: second-round assets are valued off the
#     round-2 slot arrays; hypothetical protections / swaps remain a
#     first-round-only feature (R2 obligations are modeled as outrights).
#
# Tabs:
...
# Second-round seats (overall slots 31-60) per original team / year. NULL when
# loading an older dashboard_data.rds built before round-2 support.
...
# Per-asset round lookup ("R1"/"R2"). Older rds files predate the round column;
# treat everything as first-round in that case.
asset_round_lookup <- if (!is.null(pick_assets$round)) {
  setNames(as.character(pick_assets$round), pick_assets$asset_id)
} else {
  setNames(rep("R1", nrow(pick_assets)), pick_assets$asset_id)
}
asset_round_of <- function(aid) {
  r <- unname(asset_round_lookup[aid])
  ifelse(is.na(r), "R1", r)
}
...
# The slot ceiling is detected from the stored mu_ columns (60 for the TPI
# build, 30 for legacy WS exports).
MAX_SLOT <- {
  mu_idx <- suppressWarnings(as.integer(
    sub("^mu_", "", grep("^mu_\\d+$", colnames(sim_curve_par_draws), value = TRUE))
  ))
  if (length(mu_idx) > 0) max(mu_idx, na.rm = TRUE) else 60L
}
...
slot_value_vec <- function(slot_vec) {
  ...
  slot_clamped <- pmin(pmax(slot_vec, 1L), MAX_SLOT)
  slot_index <- ifelse(is.na(slot_clamped), 1L, slot_clamped)

  mu_cols <- paste0("mu_", 1:MAX_SLOT)
  ...
  # Backward-compatible fallback for older dashboard_data.rds files. The TPI
  # build also stores delta_r2 (round-2 level shift); apply it for slots > 30
  # when present so the parametric fallback matches the production curve.
  a  <- sim_curve_par_draws[, "alpha"]
  b  <- sim_curve_par_draws[, "beta"]
  g  <- sim_curve_par_draws[, "gamma"]
  d2 <- if ("delta_r2" %in% colnames(sim_curve_par_draws)) {
    sim_curve_par_draws[, "delta_r2"]
  } else 0
  out <- a / (slot_clamped ^ b) + g + d2 * (slot_clamped > 30L)
  out[is.na(slot_clamped)] <- NA_real_
  out
}
...
# Keeps negative TPI values intact and only removes missing / non-finite values.
...
value_mode_choices <- c(
  "4-year TPI player outcomes" = "outcome",
  "Expected Asset Value" = "ev"
)

value_mode_label <- function(mode) {
  if (identical(mode, "ev")) "Expected Asset Value" else "4-year TPI player outcomes"
}
...
ui <- page_navbar(
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        p(style = "font-size:11px; color:#888; line-height:1.6;",
          ...
          "(4-year Total Points Impact) summed over 2026-2032, moving from the old ",
  ...
  nav_panel(
    ...
    card(class = "mt-2",
      card_header("3-2-1 Published-Odds Validation"),
      DTOutput("lottery_validation_table")
    ),
  ...
  nav_panel(
    ...
    layout_columns(
      col_widths = c(7, 5),
      card(
        card_header("Draft Pick Value Curve (4-yr Total Points Impact, picks 1-60)"),
...
server <- function(input, output, session) {
  ...
  output$impact_title <- renderText({
    sprintf("Change in %s (4-yr TPI scale, 2026-2032)", value_mode_label(input$impact_value_mode))
  })
  ...
  output$impact_chart <- renderPlotly({
    ...
    plot_ly(
      ...
      hovertemplate = "<b>%{y}</b><br>Delta: %{x:.1f} TPI<extra></extra>"
    ) %>%
      plotly_dark(
        xaxis = list(title = sprintf("Delta %s (4-yr TPI scale)", value_mode_label(input$impact_value_mode)),
                     zerolinecolor = "#444"),
  ...
  output$compare_title <- renderText({
    ...
    switch(input$compare_metric,
      total    = sprintf("Total Portfolio Value — %s (4-yr TPI scale, 2026-2032)", basis),
      quality  = sprintf("Best Single Pick — %s (4-yr TPI scale)", basis),
  ...
  output$lottery_line <- renderPlotly({
    ...
    official <- official_321_tier_odds
    ...
    if (nrow(official) > 0) {
      p <- p %>%
        add_markers(data = official,
                    x = ~seed_midpoint, y = ~official_expected_pick,
                    name = "Published 3-2-1 table",
                    marker = list(color = "#e5e7eb", size = 10, symbol = "x"),
                    hovertemplate = "%{text}<extra></extra>",
                    text = ~sprintf("%s<br>Published avg pick: %.1f", lottery_tier_label, official_expected_pick))
    }
  ...
  output$lottery_bar <- renderPlotly({
    ...
    if (nrow(official) > 0) {
      p <- p %>%
        add_markers(data = official,
                    x = ~seed_midpoint, y = ~(official_prob_no1 * 100),
                    name = "Published 3-2-1 table",
                    marker = list(color = "#e5e7eb", size = 10, symbol = "x"),
                    hovertemplate = "%{text}<extra></extra>",
                    text = ~sprintf("%s<br>Published P(#1): %.1f%%", lottery_tier_label, official_prob_no1 * 100))
    }
    ...
    p %>%
      plotly_dark(
        ...
        legend = list(x = 0.60, y = 0.95, font = list(size = 10))
      )
  })
  ...
  output$full_table <- renderDT({
    tbl <- full_table_source() %>%
      ...
      select(Team = team, Tier, Record, `#Pk`, Current, `3-2-1`,
             `D TPI`, `D%`, `s Cur`, `s New`, `90% Cur`, `90% New`)
    datatable(
      ...
      formatStyle("D TPI", color = styleInterval(0, c("#ef4444", "#10b981")))
  })
    ...
    card(class = "mt-2",
      ...
      card_body(style = "font-size:12px; color:#bbb; line-height:1.8;",
        ...
        tags$div(tags$strong(
          ...
          sprintf("Impact %+.1f TPI (%+.1f%%) | sigma change %+.1f | quality %+.2f",
  ...
  output$pick_curve_plot <- renderPlotly({
    pc <- pick_curve
    # Column names: TPI build writes expected_tpi/tpi_sd; legacy rds files
    # wrote expected_war/war_sd. Normalize so the plot works with either.
    if (!"expected_tpi" %in% names(pc) && "expected_war" %in% names(pc)) {
      pc <- pc %>% rename(expected_tpi = expected_war, tpi_sd = war_sd)
    }
    show_r2_boundary <- max(pc$pick, na.rm = TRUE) > 30

    p <- plot_ly(pc, x = ~pick) %>%
      add_ribbons(ymin = ~(expected_tpi - tpi_sd),
                  ymax = ~(expected_tpi + tpi_sd),
                  name = "+/- 1 SD",
    ...
    p <- p %>%
      add_lines(y = ~expected_tpi, name = "Posterior mean",
    ...
    if (show_r2_boundary) {
      shapes <- list(list(
        type = "line", x0 = 30.5, x1 = 30.5, y0 = 0, y1 = 1,
        yref = "paper",
        line = list(color = "#9ca3af", width = 1, dash = "dash")
      ))
      annotations <- list(list(
        x = 31, y = 0.97, xref = "x", yref = "paper",
        text = "Round 2", showarrow = FALSE, xanchor = "left",
        font = list(color = "#9ca3af", size = 10)
      ))
    }

    p %>%
      plotly_dark(
        xaxis = list(title = "Draft Pick (overall)", dtick = 5),
        yaxis = list(title = "4-year Total Points Impact"),
  ...
  output$validation_panel <- renderUI({
    ...
    tags$div(style = "font-size:11px; color:#bbb; line-height:1.7;",
      ...
      if (!is.null(pm$delta_r2)) tags$div(sprintf(
        "round-2 offset delta_r2=%.1f (%s in production)",
        pm$delta_r2,
        ifelse(isTRUE(pm$use_r2_offset_production), "ACTIVE", "prior-only")
      )),
      ...
      # TPI build reports sigma at picks 1/10/30/45/60; legacy WS exports
      # reported 1/5/10/30. Show whichever is present.
      if (!is.null(pm$sigma_pick_45)) tags$div(sprintf(
        "sigma[pick]: #1 %.1f | #10 %.1f | #30 %.1f | #45 %.1f | #60 %.1f",
        pm$sigma_pick_1, pm$sigma_pick_10, pm$sigma_pick_30,
        pm$sigma_pick_45, pm$sigma_pick_60
      )),
      if (is.null(pm$sigma_pick_45) && !is.null(pm$sigma_pick_1)) tags$div(sprintf(
  ...
  output$sp_headline <- renderUI({
    ...
    tags$div(
      metric_row(
        "4-year TPI player-outcome impact",
        s_out,
        "sampled player-level 4-yr TPI outcomes"
      ),
      metric_row(
        "Expected asset value impact",
        s_ev,
        "posterior mean slot value, 4-yr TPI scale"
      )
    )
  })
  ...
  single_pick_density_plot <- function(cur, new, stats_row, x_title, hover_label) {
    ...
    plot_ly() %>%
      add_lines(data = cur_density, x = ~x, y = ~density,
                ...
                hovertemplate = sprintf("Current %s<br>TPI: %%{x:.1f}<br>Density: %%{y:.3f}<extra></extra>", hover_label)) %>%
      add_lines(data = new_density, x = ~x, y = ~density,
                name = "3-2-1", fill = "tozeroy",
                line = list(color = "#f59e0b", width = 2.5),
                hovertemplate = sprintf("3-2-1 %s<br>TPI: %%{x:.1f}<br>Density: %%{y:.3f}<extra></extra>", hover_label)) %>%
      layout(
  ...
  output$sp_dist_ev <- renderPlotly({
    ...
    single_pick_density_plot(
      ...
      x_title = "Expected asset value (4-yr TPI scale)",
      hover_label = "EV"
    )
  })
  ...
  output$sp_dist_outcome <- renderPlotly({
    ...
    single_pick_density_plot(
      ...
      x_title = "Realized 4-yr TPI outcome",
      hover_label = "outcome"
    )
  })
  ...
  # Round of a display entitlement: R2 if any member asset is a second-round
  # pick (R2 entitlements are always one-to-one, so this is unambiguous).
  display_round_of <- function(display_id) {
    ids <- display_member_ids(display_id)
    if (length(ids) > 0 && any(asset_round_of(ids) == "R2")) "R2" else "R1"
  }
  ...
  obligation_controls <- function(ids, side) {
    ...
    tags$div(style = "border-top:1px solid #1a1a2a; margin-top:8px; padding-top:8px;",
      ...
      lapply(seq_len(nrow(rows)), function(i) {
        ...
        locked <- r$year == 2026
        is_r2  <- display_round_of(did) == "R2"
        eligible <- display_is_single_leg(did) && !locked && !is_r2
        tags$div(style = "margin-bottom:8px; font-size:11px; color:#aaa;",
          tags$div(tags$strong(r$label),
            ...
            if (is_r2 && !locked) tags$span(style = "color:#f59e0b;",
                                            " (2nd-round pick — modeled as outright; hypothetical protections/swaps are first-round only)"),
            if (!eligible && !locked && !is_r2) tags$span(style = "color:#f59e0b;",
  ...
  #   2. Realized player outcome: sampled player-level 4-year TPI draws.
  ...
  # The receiving team's own slot in a given year, on the right round's array.
  # R2 swaps are not user-attachable, but the helper still routes correctly if
  # called for an R2 asset; older rds files lack the R2 arrays and return NA
  # (any swap math then collapses to zero incremental value).
  receiver_slot_vec <- function(receiver, yr, round = "R1") {
    yc <- as.character(yr)
    arr <- if (identical(round, "R2")) team_slot2_new_draws else team_slot_new_draws
    if (is.null(arr) ||
        is.null(dimnames(arr)[[2]]) ||
        !receiver %in% dimnames(arr)[[2]] ||
        !yc %in% dimnames(arr)[[3]]) {
      return(rep(NA_real_, nrow(asset_new_draws)))
    }
    arr[, receiver, yc]
  }
  ...
  sent_pick_ev_value <- function(aid, side, receiver, ctrl_id = aid) {
    ...
    # Hypothetical protections / swaps are a FIRST-ROUND-ONLY feature: the UI
    # never renders the controls for R2 picks, and this guard keeps any stale
    # input state from leaking into an R2 valuation.
    if (asset_round_of(aid) == "R2") return(base)
    ...
    if (swap) {
      og_slot <- asset_slot_new_draws[, aid]
      rc_slot <- receiver_slot_vec(receiver, r$year, "R1")
  ...
  receiver_raw_outcome_vec <- function(receiver, yr, round = "R1") {
    cand <- pick_assets %>%
      filter(.data$year == as.integer(.env$yr),
             .data$original_team == .env$receiver)
    if (!is.null(cand$round)) {
      cand <- cand %>% filter(.data$round == .env$round)
    }
    if (nrow(cand) > 0) {
    ...
    rc_slot <- receiver_slot_vec(receiver, yr, round)
    if (any(!is.na(rc_slot))) {
      out <- slot_value_vec(rc_slot)
      out[is.na(out)] <- 0
      return(out)
    }
  ...
  sent_pick_outcome_value <- function(aid, side, receiver, ctrl_id = aid) {
    ...
    # First-round-only guard (see sent_pick_ev_value).
    if (asset_round_of(aid) == "R2") return(base)
    ...
    if (swap) {
      ...
      raw_rc <- receiver_raw_outcome_vec(receiver, r$year, "R1")
      eligible <- pick_conveys_app(og_slot, prot)
      sent_pick_is_better <- !is.na(og_slot) & !is.na(rc_slot) & og_slot < rc_slot
  ...
  output$tm_verdict <- renderUI({
    ...
    outcome_prob_text <- if (pTie_outcome > 0.001) {
      sprintf("P(%s's received pick(s) produce more realized 4-yr TPI) = %.0f%% | P(%s) = %.0f%% | tie = %.0f%%",
              input$tm_teamA, 100 * pA_outcome,
              input$tm_teamB, 100 * pB_outcome,
              100 * pTie_outcome)
    } else {
      sprintf("P(%s's received pick(s) produce more realized 4-yr TPI) = %.0f%% | P(%s) = %.0f%%",
              input$tm_teamA, 100 * pA_outcome,
              input$tm_teamB, 100 * pB_outcome)
    }
    ...
    tags$div(style = "font-size:12px; color:#bbb; line-height:1.8;",
      tags$div(style = "font-size:14px;",
        ...
        sprintf(" — %+.1f TPI to %s", edge_val, edge_team)),
      tags$div(sprintf("90%% posterior interval on expected asset edge to %s: [%+.1f, %+.1f] TPI",
      ...
      tags$div(sprintf("Mean realized-outcome net to %s: %+.1f TPI | 90%% interval: [%+.1f, %+.1f] TPI",
                       input$tm_teamA, eA_outcome, q05_outcome, q95_outcome)),
      tags$div(outcome_prob_text),
      tags$div(style = "color:#888; margin-top:6px;",
        sprintf("A sends %d pick(s); B sends %d pick(s). EV uses posterior mean slot curves; realized outcome uses sampled player-level 4-yr TPI draws, so a later pick can still beat an earlier pick ex post.",
  ...
  trade_density_plot <- function(x, line_color, fill_color, x_title) {
    ...
    plot_ly() %>%
      add_lines(data = ddf, x = ~x, y = ~density,
                ...
                hovertemplate = "Net TPI: %{x:.1f}<br>Density: %{y:.3f}<extra></extra>") %>%
      layout(
        paper_bgcolor = "rgba(0,0,0,0)", plot_bgcolor = "#0f0f1a",
        font = list(family = "IBM Plex Mono", color = "#999"),
        xaxis = list(title = x_title,
  ...
  output$tm_dist_ev <- renderPlotly({
    ...
    trade_density_plot(
      ...
      x_title = sprintf("Net expected 4-yr TPI transferred to %s (>0 favors %s)",
  ...
  output$tm_dist_outcome <- renderPlotly({
    ...
    trade_density_plot(
      ...
      line_color = "#7c3aed",
      fill_color = "rgba(14,165,233,0.35)",
      x_title = sprintf("Net realized 4-yr TPI transferred to %s (>0 favors %s)",
                        input$tm_teamA, input$tm_teamA)
    )
  })
