...
ui <- page_navbar(
  ...
  tags$head(tags$style(HTML("
    ...
    .full-table-page:has(#team_detail .team-detail-card) .full-table-card {
      flex: 0 0 auto;
    }
    .full-table-page:has(#team_detail .team-detail-card) #full_table_wrapper .dataTables_scrollBody {
...
server <- function(input, output, session) {
  ...
  output$impact_chart <- renderPlotly({
    ...
    df <- portfolio_quality_quantity_summary_app(
      ...
      mutate(
        ...
        size_scale = rescale_numeric_app(.data$size_metric, c(0.78, 1.55)),
        marker_size = 16 + 24 * (.data$size_scale - min(.data$size_scale, na.rm = TRUE)) /
          pmax(diff(range(.data$size_scale, na.rm = TRUE)), 0.001),
        arrow_width = 1.8 + 4.2 * (.data$size_scale - min(.data$size_scale, na.rm = TRUE)) /
          pmax(diff(range(.data$size_scale, na.rm = TRUE)), 0.001),
        active = is.null(selected_team) | .data$team == selected_team,
        trace_opacity = if_else(.data$active, 0.92, 0.13),
        logo_opacity = if_else(.data$active, 0.98, 0.20),
        hover_text = sprintf(
          paste0(
            "<b>%s</b><br>",
            "Δ Total: %+.2f %s<br>",
    ...
    x_vals <- c(df$cur_x, df$new_x)
    y_vals <- c(df$cur_y, df$new_y)
    x_ref <- if (identical(x_mode, "delta")) 0 else median(x_vals, na.rm = TRUE)
    y_ref <- median(y_vals, na.rm = TRUE)
    x_rng <- range(c(x_vals, x_ref), na.rm = TRUE)
    y_rng <- range(c(y_vals, y_ref), na.rm = TRUE)
    ...
    for (i in seq_len(nrow(df))) {
      row <- df[i, ]
      col <- if (row$delta_total_value >= 0) "#10b981" else "#ef4444"
      p <- p %>%
        add_segments(
          data = row,
          x = ~cur_x, xend = ~new_x,
          y = ~cur_y, yend = ~new_y,
          customdata = ~team,
          line = list(color = col, width = row$arrow_width),
          opacity = row$trace_opacity,
          text = ~hover_text,
          hovertemplate = "%{text}<extra></extra>",
          showlegend = FALSE
        )
    ...
    x_title <- if (identical(x_mode, "delta")) {
      paste0("Δ ", unit, " per pick")
    } else {
      paste0("Average ", unit, " per pick")
    }
  ...
  render_pick_movers_dt <- function(value_mode) {
    tryCatch({
      ...
      delta_col <- if (ncol(tbl) >= 2) names(tbl)[ncol(tbl) - 1] else tail(names(tbl), 1)
      dt <- datatable(
        tbl,
        rownames = FALSE,
        escape = TRUE,
