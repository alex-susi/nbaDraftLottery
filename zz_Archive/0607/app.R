...
if (!file.exists("01_data/dashboard_data.rds")) {
  stop("dashboard_data.rds not found. Run nba_lottery.R first.")
}

dd <- readRDS("01_data/dashboard_data.rds")
...
# It keeps negative Win Shares intact and only removes missing / non-finite values.
# Structural zeros from non-conveyance are still valid observations; they are not
# discarded or manually floored.
  ...
  nav_panel(
    ...
    div(class = "tm-page",
      ...
      tags$p(class = "tm-help",
        ...
        "Existing grouped swap/ranked-pool entitlements and locked 2026 picks use their encoded obligations. ",
        "Protections in the 12-15 band are disallowed under the approved rules and are not offered."
      ),
      layout_columns(
        col_widths = c(6, 6),
        card(
...
server <- function(input, output, session) {
  ...
  output$sp_dist <- renderPlotly({
    ...
    plot_ly() %>%
      add_lines(data = cur_density, x = ~x, y = ~density,
                ...
                hovertemplate = "Current<br>WS: %{x:.1f}<br>Density: %{y:.3f}<extra></extra>") %>%
      add_lines(data = new_density, x = ~x, y = ~density,
                name = "3-2-1", fill = "tozeroy",
                line = list(color = "#f59e0b", width = 2.5),
                hovertemplate = "3-2-1<br>WS: %{x:.1f}<br>Density: %{y:.3f}<extra></extra>") %>%
      layout(
        paper_bgcolor = "rgba(0,0,0,0)", plot_bgcolor = "#0f0f1a",
        font = list(family = "IBM Plex Mono", color = "#999"),
        xaxis = list(title = "4-yr Win Shares", gridcolor = "#1a1a2a", zerolinecolor = "#333"),
  ...
  underlying_pick_ev_with_attachment <- function(aid, side, receiver, display_id) {
    ...
    # Hypothetical protections / swaps are always available for eligible picks.
    ...
    if (swap) {
      # A traded pick and a swap right are not the same asset.
      # If receiver gets an outright pick, they receive the full value of og_slot.
      # If receiver gets only a swap right, they already keep their own pick unless
      # the sent pick is better. The asset being traded is therefore the
      # INCREMENTAL option value: max(V(sent pick) - V(receiver own pick), 0),
      # not V(best of the two picks). The old V(best) logic double-counted the
      # receiver's own pick and could make a swap right look more valuable than
      # outright conveyance.
      og_slot <- asset_slot_new_draws[, aid]
  ...
  receiver_raw_outcome_vec <- function(receiver, yr) {
    # asset_raw_new_draws stores the raw player-outcome draw for each original
    # team's pick before conveyance. Because every original team has an own-pick
    # internal asset row, we can recover the receiver's own pick outcome and use
    # it as the give-up leg in a hypothetical swap right.
    cand <- pick_assets %>%
  ...
  underlying_pick_outcome_with_attachment <- function(aid, side, receiver, display_id) {
    ...
    if (swap) {
      # Realized outcome for a swap right should also be incremental. The swap
      # is exercised based on slot order, not hindsight player outcome, so the
      # realized increment can be negative if the better slot produces the worse
      # player. If the swap is not exercised, the traded asset is worth 0.
      og_slot <- asset_slot_new_draws[, aid]
      rc_slot <- team_slot_new_draws[, receiver, yc]
  ...
  output$tm_verdict <- renderUI({
    ...
    obl <- " Hypothetical protections / swaps are available for eligible single-leg picks; swap rights are valued as incremental option value relative to the receiver's own pick, while realized-outcome draws apply protections and the ex-post give-up leg directly."
  ...
  trade_density_plot <- function(x, trace_name, line_color, hover_label) {
    dens <- density_curve_df(x)
    plot_ly() %>%
      add_lines(data = dens, x = ~x, y = ~density,
                name = trace_name, fill = "tozeroy",
                line = list(color = line_color, width = 2.5),
                hovertemplate = sprintf("%s<br>Net WS: %%{x:.1f}<br>Density: %%{y:.3f}<extra></extra>", hover_label)) %>%
      layout(
        paper_bgcolor = "rgba(0,0,0,0)", plot_bgcolor = "#0f0f1a",
        font = list(family = "IBM Plex Mono", color = "#999"),
  ...
  output$tm_dist_ev <- renderPlotly({
    ...
    trade_density_plot(
      d$net_to_A_ev,
      trace_name = "Expected asset value",
      line_color = "#f59e0b",
      hover_label = "Expected asset value"
    )
  })
  ...
  output$tm_dist_outcome <- renderPlotly({
    ...
    trade_density_plot(
      d$net_to_A_outcome,
      trace_name = "Realized outcome",
      line_color = "#6d28d9",
      hover_label = "Realized outcome"
    )
  })
