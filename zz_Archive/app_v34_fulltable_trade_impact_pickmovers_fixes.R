  ...
  pick_movers_table <- function(value_mode, year_filter, round_filter, team_filter = "All") {
    ...
    if (is.null(rows) || nrow(rows) == 0L) {
      return(tibble(
        ...
        Pick = character(), Current = numeric(), `3-2-1` = numeric(), `Δ EPV` = numeric(), `Δ EPV %` = character()
      ))
    }
    ...
    rows %>%
      mutate(
        ...
        delta_pct_raw = ifelse(abs(.data$cur_mean) > 1e-9, .data$delta / .data$cur_mean * 100, NA_real_),
        `Δ EPV %` = ifelse(is.na(.data$delta_pct_raw), "—", sprintf("%+.1f%%", .data$delta_pct_raw))
      ) %>%
  ...
  output$pm_ev_table <- renderDT({
    ...
    datatable(
      ...
      options = list(
        ...
        scrollX = TRUE,
        autoWidth = TRUE,
        columnDefs = list(list(className = "dt-right", targets = c(2, 3, 5, 6, 7, 8)))
      )
    ) %>%
      formatStyle("Δ EPV", color = styleInterval(0, c("#ef4444", "#10b981")))
  }, server = FALSE)
    ...
    signed_metric_card <- function(mean_to_A, q05_to_A, q95_to_A, label) {
      ...
      tags$div(style = paste0(
        ...
        tags$div(style = "display:flex; align-items:center; gap:10px; font-size:30px; line-height:1.1; font-weight:850; color:#10b981; text-align:left;",
                 team_logo_img_app(to_team, size = 58),
    ...
    best_outcome_card <- function() {
      tags$div(style = sprintf(
        ...
        tags$div(style = sprintf("display:flex; align-items:center; gap:10px; font-size:30px; line-height:1.1; font-weight:850; color:%s; text-align:left;", best_col),
                 team_logo_img_app(best_team, size = 58),
