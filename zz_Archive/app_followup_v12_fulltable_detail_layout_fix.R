...
ui <- page_navbar(
  ...
  tags$head(tags$style(HTML("
    ...
    body.full-detail-open #full_table_wrapper .dataTables_scrollBody {
    ...
    body.full-detail-open .full-table-card {
      flex: 0 0 auto;
    }
  ...
  ")),
    tags$script(HTML("
      $(document).on('shiny:inputchanged', function(event) {
        if (event.name === 'full_table_rows_selected') {
          var v = event.value;
          var open = false;
          if (Array.isArray(v)) {
            open = v.length > 0;
          } else {
            open = v !== null && typeof v !== 'undefined' && String(v).length > 0;
          }
          document.body.classList.toggle('full-detail-open', open);
          setTimeout(function() {
            try { $(window).trigger('resize'); } catch(e) {}
            try { $('#full_table table').DataTable().columns.adjust(); } catch(e) {}
          }, 80);
        }
      });
    "))
