################################################################################
# NBA 3-2-1 Lottery Reform — Shiny Dashboard (v23)
# VERSION NOTE:
#   - Pick Movers: tables are now sortable by every column (DT); added a "Year"
#     column between Owner and Round.
#   - Full Table: the detail panel now stays fully visible without its own
#     scrollbar; the table shrinks vertically to make room so the table is the
#     ONLY scrollable element on the tab.
#   - Impact: removed the "How to read this" sidebar blurb.
#   - Markov + Curve: removed the two per-round pick-value curves; only the
#     combined picks 1-60 curve remains.
#
...
ui <- page_navbar(
  ...
  tags$head(tags$style(HTML("
    ...
    /* The table scroll-body height is managed dynamically by the
       fullTableFit controller (see script below). It shrinks when the
       detail panel is open so the table is the ONLY scrollable element. */
    #full_table_wrapper .dataTables_scrollBody {
      overflow-y: auto !important;
      height: auto !important;
      max-height: var(--full-table-scroll, calc(100vh - 315px)) !important;
    }
    #team_detail,
    #team_detail .card,
  ...
  tags$head(tags$script(HTML("
    (function() {
      ...
      function fit() {
        var scrollBody = document.querySelector('#full_table_wrapper .dataTables_scrollBody');
        ...
        // Top of the scroll body relative to the viewport.
        var top = scrollBody.getBoundingClientRect().top;

        // Reserve space below: the detail panel (with a gap above it) plus a
        // small bottom margin so nothing is flush against the window edge.
      ...
      // Re-fit shortly after load and whenever the layout can change.
      ...
      // Bootstrap a few delayed passes for the initial async render.
...
server <- function(input, output, session) {
  ...
  output$full_table <- renderDT({
    ...
    datatable(
      ...
      options = list(pageLength = 30, paging = FALSE, dom = "t", ordering = TRUE,
                     scrollY = "calc(100vh - 315px)", scrollCollapse = FALSE, scrollX = TRUE,
                     columnDefs = list(list(className = "dt-right", targets = 3:10))),
      callback = htmlwidgets::JS("
        table.on('click', 'tbody tr', function() {
          setTimeout(function() {
            var el = document.getElementById('team_detail');
  ...
  pick_movers_dt <- function(value_mode) {
    tbl <- pick_movers_table(value_mode, input$pm_year %||% "All", input$pm_round %||% "All")
    if (is.null(tbl) || nrow(tbl) == 0L) {
      return(datatable(
        tibble(Message = "No picks match the selected filters."),
        rownames = FALSE, options = list(dom = "t", ordering = FALSE),
        colnames = ""
      ))
    }

    # Keep PickInfo as the LAST column so we can (a) hide it from view and
    # (b) read it back per row in rowCallback for the hover tooltip. Reading it
    # from the row's own data (rather than a captured index) keeps the tooltip
    # correct after the user sorts the table.
    visible_names <- setdiff(names(tbl), "PickInfo")
    delta_col <- visible_names[length(visible_names)]
    show_tbl <- tbl[, c(visible_names, "PickInfo"), drop = FALSE]

    pickinfo_idx <- length(show_tbl) - 1L            # 0-indexed position of PickInfo
    right_cols <- intersect(c("Year", "Round", "Current", "3-2-1", delta_col), visible_names)
    right_targets <- match(right_cols, visible_names) - 1L  # 0-indexed for DT
    ...
    dt <- datatable(
      ...
      options = list(
        ...
        scrollCollapse = TRUE,
        columnDefs = list(
          list(visible = FALSE, targets = pickinfo_idx),
          list(className = "dt-right", targets = right_targets)
        ),
        # Attach the row's own PickInfo value as a hover title on the <tr>.
        rowCallback = htmlwidgets::JS(sprintf(
          "function(row, data) { var info = data[%d]; if (info) { $(row).attr('title', info); } }",
          pickinfo_idx
        ))
      )
    ) %>%
      formatStyle(
        delta_col,
        color = styleInterval(0, c("#ef4444", "#10b981"))
      )
