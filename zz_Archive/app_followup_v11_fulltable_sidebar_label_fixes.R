...
server <- function(input, output, session) {
  ...
  output$full_table <- renderDT({
    ...
    has_detail <- length(sel) > 0
    scroll_y <- if (has_detail) "220px" else "calc(100vh - 245px)"
    ...
    datatable(
      tbl, selection = list(mode = "single", selected = sel, target = "row"), rownames = FALSE, escape = FALSE,
