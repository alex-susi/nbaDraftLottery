...
sample_pick_value_bayes <- function(pos, draw_idx = sample(nrow(pick_draws), 1)) {
  ...
  mu <- d$alpha * exp(-d$beta * (pos - 1)) + d$gamma
...
value_2026 <- function(d_pick) {
  ...
  for (r in seq_len(nrow(actual_2026_order))) {
    own <- actual_2026_order$owner[r]
    val <- sample_pick_value(actual_2026_order$slot[r], draw_idx = d_pick)
    v[own]     <- v[own] + val
    nbest[own] <- max(nbest[own], val)
    ct[own]    <- ct[own] + 1L
  }
  list(v = v, best = nbest, n = ct)
}
  ...
  for (yr in FIRST_PROJECTED_DRAFT:LAST_PROJECTED_DRAFT) {
    ...
    for (j in seq_len(nrow(yr_out))) {
      ...
      if (!is.na(slot_c[og]) && pick_conveys(slot_c[og], prot)) {
        val <- sample_pick_value(slot_c[og], draw_idx = d_pick)
        tv_c[ow] <- tv_c[ow] + val
        tn_c[ow] <- tn_c[ow] + 1L
        tb_c[ow] <- max(tb_c[ow], val)
      }
      if (!is.na(slot_n[og]) && pick_conveys(slot_n[og], prot)) {
        val <- sample_pick_value(slot_n[og], draw_idx = d_pick)
        tv_n[ow] <- tv_n[ow] + val
        tn_n[ow] <- tn_n[ow] + 1L
        tb_n[ow] <- max(tb_n[ow], val)
      }
    }
