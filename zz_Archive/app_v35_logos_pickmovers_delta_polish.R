################################################################################
# NBA 3-2-1 Lottery Reform — Shiny Dashboard (v34)
...
ui <- page_navbar(
  ...
  tags$head(tags$style(HTML("
    ...
    .selectize-logo-option, .selectize-logo-item {
      display: inline-flex;
      align-items: center;
      gap: 6px;
      min-width: 0;
  ...
  nav_panel(
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        checkboxInput("impact_show_uncertainty", "View EPV Uncertainty", value = FALSE),
...
# ---- Tab 3 ----
  ...
  nav_panel(
    ...
    div(class = "pm-page",
      tags$style(HTML("
        .pm-page { min-height: calc(100vh - 92px); }
        .pm-page table.dataTable { font-size: 12px; color:#e6e8ef; }
        .pm-page .card,
        .pm-page .bslib-card,
        .pm-page .card-body,
      ...
      layout_sidebar(
        ...
        layout_columns(
          ...
          card(
            ...
            DTOutput("pm_ev_table", height = "calc(100vh - 255px)")
          )
        )
      )
    )
  ...
  nav_panel(
    ...
    div(class = "tm-page",
      ...
      ")),
      div(style = "display:flex; justify-content:center; margin: 2px 0 12px;",
...
server <- function(input, output, session) {
  ...
  output$impact_chart <- renderPlotly({
    ...
    if (isTRUE(input$impact_show_uncertainty) && all(c("new_avg_quality_q10", "new_avg_quality_q90") %in% names(df))) {
      ci_df <- df %>% filter(is.finite(.data$new_avg_quality_q10), is.finite(.data$new_avg_quality_q90))
      if (nrow(ci_df) > 0) {
        p <- p %>% add_segments(
          data = ci_df,
          x = ~new_avg_quality_q10, xend = ~new_avg_quality_q90,
          y = ~y, yend = ~y,
          line = list(color = "rgba(229,231,235,0.30)", width = 7),
    ...
    shapes <- list(
      list(type = "path",
           path = sprintf("M %f,%f L %f,%f L %f,%f Z", x_rng[1] - pad_x, y_rng[1] - pad_y, x_rng[2] + pad_x, y_rng[1] - pad_y, x_rng[1] - pad_x, y_rng[2] + pad_y),
           fillcolor = "rgba(239,68,68,0.055)", line = list(color = "rgba(239,68,68,0)"), layer = "below"),
      list(type = "path",
           path = sprintf("M %f,%f L %f,%f L %f,%f Z", x_rng[2] + pad_x, y_rng[2] + pad_y, x_rng[2] + pad_x, y_rng[1] - pad_y, x_rng[1] - pad_x, y_rng[2] + pad_y),
           fillcolor = "rgba(16,185,129,0.055)", line = list(color = "rgba(16,185,129,0)"), layer = "below"),
      list(type = "line", x0 = x_ref, x1 = x_ref, y0 = y_rng[1] - pad_y, y1 = y_rng[2] + pad_y,
           line = list(color = "rgba(255,255,255,0.20)", dash = "dash")),
  ...
  output$full_table <- renderDT({
    ...
    datatable(
      ...
      formatStyle("Δ EPV", color = styleInterval(c(-0.05, 0.05), c("#ef4444", "#d0d0d0", "#10b981")))
  }, server = FALSE)
  ...
  output$team_detail <- renderUI({
    ...
    html_tbl <- function(tbl, first_header, empty_msg = "No picks", first_color = "#aaa") {
      ...
      tags$table(style = "width:100%; border-collapse:collapse; font-size:11px; table-layout:fixed;",
        tags$thead(tags$tr(
          tags$th(style = sprintf("text-align:left; color:%s; padding:4px; width:48%%;", first_color), first_header),
    ...
    breakout_tbl_round <- function(tbl, round_no) {
      ...
      tags$table(style = "width:100%; border-collapse:collapse; font-size:11px; margin-top:5px; table-layout:fixed;",
        tags$thead(tags$tr(
          tags$th(style = "text-align:left; color:#aaa; padding:4px; width:36%;", paste0("Round ", round_no)),
    ...
    card(class = "team-detail-card",
      ...
      card_body(style = "font-size:12px; color:#bbb; line-height:1.45; overflow-x:hidden; padding-top:10px;",
        ...
        layout_columns(
          col_widths = c(12),
          tags$div(breakout_tbl_round(impact_tbl, 1L)),
          tags$div(breakout_tbl_round(impact_tbl, 2L))
        )
      )
    )
  })
  ...
  output$sp_obligation <- renderUI({
    ...
    tags$div(style = "font-size:11px; color:#aaa; line-height:1.55;",
      tags$div(tags$strong(style = "color:#ffffff; font-size:15px;", "Pick Details")),
      line_div(
        tags$span("Current Team: "),
        team_logo_img_app(r$owner, size = 18),
        tags$span(style = "margin-left:6px;", r$owner)
      ),
      line_div(
        tags$span(sprintf("Original Team%s: ", ifelse(str_detect(r$member_original_teams, ","), "s", ""))),
        tagList(lapply(split_abbrs(r$member_original_teams), function(tm) {
          tags$span(style = "display:inline-flex; align-items:center; gap:4px; margin-right:8px;",
            team_logo_img_app(tm, size = 18), tags$span(tm))
        }))
      ),
      if (!is.na(r$fixed_slot_display)) line_div(tags$span(style = "color:#10b981;", sprintf("Actual Pick: %s", r$fixed_slot_display))),
      line_div(sprintf("Obligation: %s", obligation_txt)),
  ...
  output$sp_headline <- renderUI({
    ...
    tags$div(
      tags$div(style = "font-size:12px; font-weight:700; color:#d0d0d0; margin:0 0 6px 2px;", "Expected Pick Value impact"),
  ...
  pick_movers_table <- function(value_mode, year_filter, round_filter, team_filter = "All") {
    ...
    if (is.null(rows) || nrow(rows) == 0L) {
      return(tibble(
        Direction = character(), Owner = character(), Year = integer(), Round = integer(),
    ...
    rows %>%
      mutate(
        ...
        `Δ EPV` = round(.data$delta, 1),
        `Δ EPV %` = round(ifelse(abs(.data$cur_mean) > 1e-9, .data$delta / .data$cur_mean * 100, NA_real_), 1)
      ) %>%
      arrange(desc(.data$delta), .data$year, .data$round, .data$owner, .data$short_label) %>%
      transmute(Direction, Owner, Year = as.integer(.data$year),
  ...
  output$pm_ev_table <- renderDT({
    ...
    datatable(
      ...
      options = list(
        ...
        autoWidth = TRUE,
        columnDefs = list(
          list(className = "dt-right", targets = c(2, 3, 5, 6, 7, 8)),
          list(targets = 7, render = JS("function(data,type,row,meta){ if(type === 'display'){ var x = parseFloat(data); if(isNaN(x)) return '—'; return (x > 0 ? '+' : '') + x.toFixed(1); } return data; }")),
          list(targets = 8, render = JS("function(data,type,row,meta){ if(type === 'display'){ var x = parseFloat(data); if(isNaN(x)) return '—'; return (x > 0 ? '+' : '') + x.toFixed(1) + '%'; } return data; }"))
        )
      )
    ) %>%
      formatStyle("Δ EPV", color = styleInterval(c(-0.05, 0.05), c("#ef4444", "#d0d0d0", "#10b981"))) %>%
  ...
  trade_density_plot <- function(x, teamA, teamB, x_title, hover_label = "Net") {
    ...
    colA <- team_primary_color_app(teamA)
    fillA <- hex_to_rgba_app(team_secondary_color_app(teamA), 0.36)
    colB <- team_primary_color_app(teamB)
    fillB <- hex_to_rgba_app(team_secondary_color_app(teamB), 0.36)
    ...
    p %>% layout(
      paper_bgcolor = "rgba(0,0,0,0)", plot_bgcolor = "#151522",
      font = list(family = "IBM Plex Mono", color = "#aaa"),
      xaxis = list(title = x_title, gridcolor = "#24243a", zerolinecolor = "#8a8f9c"),
      yaxis = list(title = "Density", gridcolor = "#24243a"),
      showlegend = FALSE,
      shapes = list(list(type = "line", x0 = 0, x1 = 0, y0 = 0, y1 = 1,
                         yref = "paper",
                         line = list(color = "#8a8f9c", dash = "dash"))))
  }
