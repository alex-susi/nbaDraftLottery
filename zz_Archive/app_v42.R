################################################################################
# NBA 3-2-1 Lottery Reform — Shiny Dashboard (v42)
# VERSION NOTE (v42 — scatterplot labels/hover logos, validation table why column, UI polish):
...
team_logo_layout_images_app <- function(df, x_col, y_col, sizex, sizey, opacity = 0.95) {
  ...
  purrr::map(seq_len(nrow(df)), function(i) {
    ...
    list(
      ...
      opacity = opacity,
      layer = "above",
      name = as.character(df$team[i]),
      templateitemname = as.character(df$team[i])
    )
  }) %>% purrr::compact()
}
...
server <- function(input, output, session) {
  ...
  output$impact_chart <- renderPlotly({
    if (identical(input$impact_view %||% "scatter", "leaderboard")) {
      ...
      logo_images <- team_logo_layout_images_app(
        ...
        x_col = "x", y_col = "y",
        sizex = max(3.2, diff(range(c(df$current_mean, df$new_mean), na.rm = TRUE)) * 0.030),
        sizey = 1.00,
        opacity = 0.98
      )
      ...
      return(
        p %>%
          ...
          logo_hover_enlarger_app(scale = 1.60)
      )
    }
    ...
    p %>%
      ...
      logo_hover_enlarger_app(scale = 1.55)
  })
  ...
  prob_box <- function(title, stat, col) {
    ...
    tags$div(style = sprintf(
      "flex:1; min-width:0; padding:12px 14px; border:1px solid #1a1a2a; border-radius:10px; border-left:4px solid %s;", col),
      tags$div(style = "font-size:13px; color:#9ca3af; white-space:nowrap;", title),
      tags$div(style = sprintf("font-size:28px; font-weight:800; color:%s; line-height:1.15;", col),
               sprintf("%.1f%%", 100 * stat$prob)),
      tags$div(style = "font-size:13px; color:#c5c8d2; white-space:nowrap;",
               sprintf("90%% CI [%.1f%%, %.1f%%]", 100 * stat$q05, 100 * stat$q95)))
  }

  prob_card_row <- function(label, cur_stat, new_stat) {
    tags$div(style = "margin-top:15px;",
      tags$div(style = "font-size:16px; font-weight:800; color:#e5e7eb; margin-bottom:8px;", label),
      tags$div(style = "display:flex; gap:12px; align-items:stretch; flex-wrap:nowrap;",
  ...
  output$sp_obligation <- renderUI({
    ...
    tags$div(style = "font-size:14px; color:#c5c8d2; line-height:1.65;",
  ...
  tm_identity_tag_app <- function(team) {
    tags$div(
      team_logo_img_app(team, size = 76),
