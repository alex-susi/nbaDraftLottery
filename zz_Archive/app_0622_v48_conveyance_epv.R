...
server <- function(input, output, session) {
  ...
  side_epv_impact_value <- function(ids, side, receiver) {
    if (is.null(ids) || length(ids) == 0) return(zero_trade_vec())
    side_initial_ev_value(ids, side, receiver) - side_out_ev_value(ids, side, receiver)
  }
  ...
  render_trade_pick_table <- function(ids, side, receiver, pad_to = NULL) {
    ...
    body_rows <- lapply(seq_len(nrow(rows)), function(i) {
      ...
      tags$tr(
        ...
        tags$td(if (eligible) selectInput(pid, NULL, choices = protection_choices, selected = input[[pid]] %||% "none", width = "112px") else tags$span(style = "color:#777;", "N/A")),
