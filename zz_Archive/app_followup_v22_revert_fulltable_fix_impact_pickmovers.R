...
server <- function(input, output, session) {
  ...
  pick_movers_html <- function(value_mode) {
    tryCatch({
      tbl <- pick_movers_table(value_mode, input$pm_year %||% "All", input$pm_round %||% "All")
      if (is.null(tbl) || nrow(tbl) == 0L) {
        return(tags$div(style = "color:#777; padding:12px;", "No picks match the selected filters."))
      }
      ...
      tags$div(
        style = "max-height:620px; overflow-y:auto; overflow-x:hidden; padding:4px 2px;",
        tags$table(
          style = "width:100%; border-collapse:collapse; font-size:12px; table-layout:fixed;",
          tags$thead(tags$tr(lapply(visible_names, function(nm) {
            align <- if (nm %in% c("Round", "Current", "3-2-1", delta_col)) "right" else "left"
            tags$th(style = sprintf("text-align:%s; color:#aaa; padding:7px 8px; border-bottom:1px solid #1a1a2a;", align), nm)
          }))),
          tags$tbody(lapply(seq_len(nrow(tbl)), function(i) {
            r <- tbl[i, ]
            tags$tr(
              title = r$PickInfo,
              style = if (i %% 2 == 0) "background:#100d20;" else "background:#090914;",
              lapply(visible_names, function(nm) {
                val <- r[[nm]][[1]]
                align <- if (nm %in% c("Round", "Current", "3-2-1", delta_col)) "right" else "left"
                color <- if (identical(nm, delta_col)) {
                  if (is.finite(as.numeric(val)) && as.numeric(val) >= 0) "#10b981" else "#ef4444"
                } else {
                  "#d0d0d0"
                }
                tags$td(style = sprintf("text-align:%s; color:%s; padding:7px 8px; vertical-align:top;", align, color), as.character(val))
              })
            )
          }))
        )
      ...
      tags$div(style = "color:#ef4444; padding:12px;", paste("Pick Movers failed:", conditionMessage(e)))
    })
  }
