...
server <- function(input, output, session) {
  ...
  output$sp_obligation <- renderUI({
    ...
    team_detail_row <- function(label, value_ui) {
      tags$div(class = "sp-team-detail-row",
        tags$div(style = "color:#aaa;", label),
    ...
    tags$div(style = "font-size:11px; color:#aaa; line-height:1.55;",
      ...
      team_detail_row(
        "Current Team:",
        tagList(team_logo_img_app(r$owner, size = 18), tags$span(r$owner))
      ),
      team_detail_row(
    ...
    datatable(
      ...
      options = list(
        ...
        columnDefs = list(
          ...
          list(targets = 8, render = JS("function(data,type,row,meta){ var txt = $('<div>').html(data).text().replace('%',''); var x = parseFloat(txt); if(type === 'sort' || type === 'type') return isNaN(x) ? 0 : x; return data; }"))
        )
      )
    )
  }, server = FALSE)
  ...
  trade_density_plot <- function(x, teamA, teamB, x_title, hover_label = "Net") {
    ...
    fillA <- hex_to_rgba_app(team_primary_color_app(teamA), 0.34)
    colB <- team_primary_color_app(teamB)
    fillB <- hex_to_rgba_app(team_primary_color_app(teamB), 0.34)
