...
pick_model <- cmdstan_model("pick_value_v2.stan")

# Player-level pick-value model. The old version fit 30 slot means using
# war_mean and war_se. The revised model fits the full drafted-player outcome
# distribution directly: one row per drafted player, outcome = first-4-year WS.
...
pick_core_vars <- c("alpha", "beta", "gamma", "sigma_base", "sigma_slope", "nu")
...
team_slot_new <- array(NA_real_, dim = c(N_SIMS, 30, n_proj_years),
                       dimnames = list(NULL, all_teams, as.character(proj_years)))
sim_curve_par <- matrix(NA_real_, N_SIMS, 6,
                        dimnames = list(NULL, c("alpha","beta","gamma",
                                                "sigma_base","sigma_slope","nu")))
...
for (sim in 1:N_SIMS) {
  ...
  sim_curve_par[sim, ] <- c(pick_draws$alpha[d_pick], pick_draws$beta[d_pick],
                            pick_draws$gamma[d_pick], pick_draws$sigma_base[d_pick],
                            pick_draws$sigma_slope[d_pick], pick_draws$nu[d_pick])
...
stan_diagnostics <- list(
  pick_model = list(
    ...
    sigma_slope = round(mean(pick_draws$sigma_slope), 4),
    ...
    curve_type  = ifelse(USE_BAYESIAN_PICK_CURVE, "Bayesian Student-t player-level Stan", "Bootstrap")
  ),
  markov_model = list(
...
dashboard_data <- list(
  ...
  metadata = list(
    ...
    model_note  = "Bayesian 5-tier Markov chain + player-level Student-t 4-yr Win Shares pick curve",
    tiers       = TIERS,
