...
server <- function(input, output, session) {
  ...
  output$team_detail <- renderUI({
    ...
    impact_tbl <- pick_rows %>%
      ...
      summarise(
        n_picks = n_distinct(.data$display_asset_id),
  ...
  pick_curve_base_plot <- function(pc, x_title, y_title = "4-Yr Win Shares") {
    ...
    p %>%
      ...
      plotly_dark(
        ...
        margin = list(l = 70, r = 190, t = 45, b = 58)
      )
  }
    ...
    if (pad_n > 0L) {
      spacer_rows <- lapply(seq_len(pad_n), function(i) {
        tags$tr(class = "tm-spacer-row",
          tags$td(style = "height:56px; padding:6px 0; border-bottom:0;", HTML("&nbsp;")),
          tags$td(style = "height:56px; padding:6px 0; border-bottom:0;", HTML("&nbsp;")),
          tags$td(style = "height:56px; padding:6px 0; border-bottom:0;", HTML("&nbsp;")),
          tags$td(style = "height:56px; padding:6px 0; border-bottom:0;", HTML("&nbsp;"))
        )
    ...
    best_outcome_card <- function() {
      tags$div(style = sprintf(
        paste0(
          ...
          "flex-direction:column; justify-content:center; height:100%; min-height:0; box-sizing:border-box;"
        ), best_col),
