################################################################################
# NBA 3-2-1 Lottery Reform — Shiny Dashboard (v25)
...
ui <- page_navbar(
  ...
  # ---- Tab 1b: Winners & Losers (dumbbell / tornado) ----
  nav_panel(
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        tags$p(class = "mono", style = "font-size:11px; color:#888; line-height:1.6;",
          ...
          "Color reflects confidence in the direction of the change: vivid green/red when the simulation is fairly sure (P of that direction \u2265 85%), a muted shade when it only leans that way (\u2265 65%), and gray when the change is within noise (closer to a coin flip). ",
...
server <- function(input, output, session) {
  ...
  output$wl_dumbbell <- renderPlotly({
    ...
    df <- base %>%
      ...
      mutate(
        ...
        # from a coin flip. This is far more discriminating at the portfolio
        # level than asking the full 80% band to clear zero (which almost never
        # happens when a team holds many picks).
        conf = pmax(.data$p_pos, 1 - .data$p_pos),
        # 3-level signal: strong / lean / noise.
        ...
        col = dplyr::case_when(
          .data$signal_level == "noise" ~ "#6b7280",
          .data$dir == "gain" & .data$signal_level == "strong" ~ "#10b981",
          .data$dir == "gain" ~ "#5eead4",   # muted teal-green for "lean" gains
          .data$signal_level == "strong" ~ "#ef4444",
          TRUE ~ "#fca5a5"                    # muted red for "lean" losses
        ),
    ...
    hover <- sprintf(
      ...
      df$dir, 100 * df$conf,
      dplyr::recode(df$signal_level, strong = "strong signal", lean = "leans this way", noise = "within noise")
    )
    ...
    # Legend proxies for the color semantics (vivid = high confidence; muted
    # shades of the same hue mean the direction only "leans" that way).
    p <- p %>%
      ...
      add_markers(x = NA, y = NA,
                  marker = list(color = "#6b7280", size = 11), name = "Within noise")
  ...
  output$full_table <- renderDT({
    ...
    signal_html <- vapply(seq_len(nrow(base)), function(i) {
      conf <- base$conf[i]
      sig <- base$is_signal[i]
      up <- base$delta_value[i] >= 0
      arrow <- if (up) "&#9650;" else "&#9660;"   # ▲ / ▼
      col <- if (!isTRUE(sig)) "#6b7280" else if (up) "#10b981" else "#ef4444"
      sprintf("<span style='color:%s; white-space:nowrap;'>%s %.0f%%</span>", col, arrow, 100 * conf)
    }, character(1))
    ...
    datatable(
      ...
      formatStyle("_noise", target = "row",
                  opacity = styleEqual(c("0", "1"), c("1", "0.55")))
  }, server = FALSE)
