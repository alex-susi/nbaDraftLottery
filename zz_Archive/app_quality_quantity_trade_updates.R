...
value_mode_unit <- function(mode) {
  if (identical(mode, "ev")) "EPV" else "4yr WS"
}
...
portfolio_quality_quantity_summary_app <- function(value_mode, year_filter = "All", round_filter = "All") {
  ...
  purrr::map_dfr(all_summary_teams, function(tm) {
    ...
    if (length(count_ids) == 0L) {
      ...
      cur_count_draw <- rowSums(display_convey_cur_draws[, count_ids, drop = FALSE], na.rm = TRUE)
      new_count_draw <- rowSums(display_convey_new_draws[, count_ids, drop = FALSE], na.rm = TRUE)
    }
    ...
    cur_avg_quality <- ifelse(cur_count > 0.01, cur_total_mean / cur_count, 0)
    new_avg_quality <- ifelse(new_count > 0.01, new_total_mean / new_count, 0)
...
pick_impact_rows_app <- function(value_mode) {
  ...
  smry %>%
    left_join(pick_display_assets, by = "display_asset_id") %>%
    mutate(
      delta = coalesce(.data$delta, .data$new_mean - .data$cur_mean),
      impact_bucket = {
        own_team <- stringr::str_detect(.data$member_original_teams %||% "", paste0("\\b", .data$owner, "\\b"))
        txt <- stringr::str_to_lower(paste(.data$group_type %||% "", .data$obligation %||% "", .data$notes %||% "", .data$label %||% ""))
        dplyr::case_when(
          own_team & !stringr::str_detect(txt, "swap|protect|conditional|complex|favorable") ~ "Own picks",
          !own_team & stringr::str_detect(txt, "outright| to ") & !stringr::str_detect(txt, "swap|protect|conditional|complex|favorable") ~ "Incoming outright picks",
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        p(style = "font-size:11px; color:#888; line-height:1.6;",
          "Each team starts at its current-system portfolio position and moves to its 3-2-1 position. ",
          "The x-axis is average value per expected pick, the y-axis is expected pick count, ",
          "bubble size is total portfolio value, and color reflects the total 3-2-1 impact.",
          br(), br(),
          "Right/up means better average pick quality and more expected picks; left/down means worse quality and fewer expected picks.")
      ),
      card(
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        selectInput("pm_round", "Round",
          choices = c("All" = "All", "Round 1" = "1", "Round 2" = "2"), selected = "All"),
        tags$p(style = "font-size:11px; color:#888; line-height:1.6;",
          "Shows the single display pick entitlements with the largest positive and negative 3-2-1 impact.")
      ),
      layout_columns(
        col_widths = c(6, 6),
        card(
          card_header("Best / Worst by Expected Pick Value"),
          DTOutput("pm_ev_table")
        ),
        card(
          card_header("Best / Worst by 4-yr Win Share Outcomes"),
          DTOutput("pm_outcome_table")
        )
      )
    )
  ),
    ...
    div(class = "tm-page",
      ...
      layout_columns(
        ...
        card(class = "tm-selection-card",
          ...
          selectizeInput("tm_picksA", "Picks Team 1 sends out",
        ...
        card(class = "tm-selection-card",
          ...
          selectizeInput("tm_picksB", "Picks Team 2 sends out",
            choices = NULL, multiple = TRUE,
            options = list(placeholder = "select one or more picks"))
        )
      ),
      layout_columns(
        col_widths = c(6, 6),
        card(class = "tm-selection-card",
          card_header("Team 1 outgoing picks"),
          uiOutput("tm_picksA_table")
        ),
        card(class = "tm-selection-card",
          card_header("Team 2 outgoing picks"),
...
server <- function(input, output, session) {
  ...
  output$impact_chart <- renderPlotly({
    df <- portfolio_quality_quantity_summary_app(
      ...
      mutate(
        ...
        hover_text = sprintf(
          paste0(
            "<b>%s</b><br>",
            "Current: %.2f %s across %.2f expected picks<br>",
            "3-2-1: %.2f %s across %.2f expected picks<br><br>",
            "Δ Total: %+.2f %s<br>",
            "Δ Avg value / pick: %+.2f<br>",
            "Δ Expected picks: %+.2f<br>",
            "P(positive impact): %.1f%%<br><br>",
            "Quality effect: %+.2f<br>",
            "Quantity effect: %+.2f<br>",
            "Interaction: %+.2f"
          ),
          .data$team,
          .data$cur_total_value, value_mode_unit(input$impact_value_mode), .data$cur_expected_picks,
          .data$new_total_value, value_mode_unit(input$impact_value_mode), .data$new_expected_picks,
          .data$delta_total_value, value_mode_unit(input$impact_value_mode),
          .data$delta_avg_quality,
          .data$delta_expected_picks,
          100 * .data$p_positive,
          .data$quality_effect,
          .data$quantity_effect,
          .data$interaction_effect
        )
      )

    x_ref <- median(c(df$cur_avg_quality, df$new_avg_quality), na.rm = TRUE)
    y_ref <- median(c(df$cur_expected_picks, df$new_expected_picks), na.rm = TRUE)
    x_rng <- range(c(df$cur_avg_quality, df$new_avg_quality, x_ref), na.rm = TRUE)
    y_rng <- range(c(df$cur_expected_picks, df$new_expected_picks, y_ref), na.rm = TRUE)
    if (!all(is.finite(x_rng)) || diff(x_rng) == 0) x_rng <- c(-0.5, 0.5) + x_ref
    if (!all(is.finite(y_rng)) || diff(y_rng) == 0) y_rng <- c(-0.5, 0.5) + y_ref

    pad_x <- max(0.25, diff(x_rng) * 0.12)
    ...
    for (sg in c("Positive", "Negative")) {
      ...
      p <- p %>%
        add_segments(
          data = sub,
          x = ~cur_avg_quality, xend = ~new_avg_quality,
          y = ~cur_expected_picks, yend = ~new_expected_picks,
        ...
        add_markers(
          data = sub,
          x = ~new_avg_quality, y = ~new_expected_picks,
          size = ~bubble_size,
          sizes = c(8, 38),
          marker = list(color = col, opacity = 0.82, line = list(color = "#0f0f1a", width = 1)),
          text = ~hover_text,
          hovertemplate = "%{text}<extra></extra>",
          name = sg,
          showlegend = TRUE
    ...
    p <- p %>%
      add_markers(
        data = df,
        x = ~cur_avg_quality, y = ~cur_expected_picks,
        marker = list(color = "#94a3b8", size = 6, opacity = 0.45, symbol = "circle-open"),
        hovertemplate = paste0("%{text}<extra></extra>"),
        text = ~sprintf("<b>%s current</b><br>Avg value / pick: %.2f<br>Expected picks: %.2f<br>Total: %.2f %s",
                        team, cur_avg_quality, cur_expected_picks, cur_total_value, value_mode_unit(input$impact_value_mode)),
        name = "Current position",
        showlegend = TRUE
      ) %>%
      add_text(
        data = df %>% arrange(desc(abs(.data$delta_total_value))) %>% slice_head(n = 12),
        x = ~new_avg_quality,
        y = ~new_expected_picks,
        text = ~team,
        textposition = "top center",
        textfont = list(size = 10, color = "#d0d0d0"),
        hoverinfo = "skip",
        showlegend = FALSE
      )
    ...
    annotations <- list(
      list(x = x_rng[2] + pad_x * 0.6, y = y_rng[2] + pad_y * 0.5, text = "High quality / high quantity", showarrow = FALSE, font = list(size = 10, color = "#10b981")),
      list(x = x_rng[2] + pad_x * 0.6, y = y_rng[1] - pad_y * 0.3, text = "High quality / low quantity", showarrow = FALSE, font = list(size = 10, color = "#aaa")),
      list(x = x_rng[1] - pad_x * 0.6, y = y_rng[2] + pad_y * 0.5, text = "Low quality / high quantity", showarrow = FALSE, font = list(size = 10, color = "#aaa")),
      list(x = x_rng[1] - pad_x * 0.6, y = y_rng[1] - pad_y * 0.3, text = "Low quality / low quantity", showarrow = FALSE, font = list(size = 10, color = "#ef4444"))
    )

    p %>%
      plotly_dark(
        xaxis = list(title = list(text = paste0("Average ", value_mode_unit(input$impact_value_mode), " per expected pick")),
                     tickformat = ".2f", range = c(x_rng[1] - pad_x, x_rng[2] + pad_x)),
        yaxis = list(title = list(text = "Expected pick count"), tickformat = ".2f",
                     range = c(y_rng[1] - pad_y, y_rng[2] + pad_y)),
        shapes = shapes,
        annotations = annotations,
        legend = list(orientation = "h", x = 0.02, y = 1.10, xanchor = "left", yanchor = "bottom"),
  ...
  output$full_table <- renderDT({
    ...
    current_col <- if (identical(mode, "ev")) "Current EPV" else "Current 4yr WS"
    new_col <- if (identical(mode, "ev")) "3-2-1 EPV" else "3-2-1 4yr WS"
    delta_col <- if (identical(mode, "ev")) "Δ EPV" else "Δ 4yr WS"
    ...
    tbl <- full_table_source() %>%
      ...
      transmute(
        ...
        `Δ%` = sprintf("%+.1f%%", .data$delta_pct),
        `P(Positive 3-2-1 Impact)` = sprintf("%.1f%%", 100 * coalesce(.data$p_positive, 0)),
  ...
  output$team_detail <- renderUI({
    ...
    impact_tbl <- pick_rows %>%
      ...
      complete(
        ...
        round = c(1L, 2L),
        fill = list(current = 0, new = 0, delta = 0)
      ) %>%
    ...
    breakout_tbl <- function(tbl) {
      tags$table(style = "width:100%; border-collapse:collapse; font-size:11px; margin-top:8px;",
        tags$thead(tags$tr(
          ...
          tags$th(style = "text-align:center; color:#aaa; padding:4px;", "Round"),
          tags$th(style = "text-align:right; color:#aaa; padding:4px;", "Current"),
          tags$th(style = "text-align:right; color:#aaa; padding:4px;", "3-2-1"),
          tags$th(style = "text-align:right; color:#aaa; padding:4px;", paste0("Δ ", unit))
        )),
        tags$tbody(lapply(seq_len(nrow(tbl)), function(i) {
          ...
          tags$tr(
            ...
            tags$td(style = "padding:4px; text-align:center;", r$round),
    ...
    card(class = "mt-2",
      ...
      card_body(style = "font-size:12px; color:#bbb; line-height:1.6;",
        tags$div(style = "color:#888; margin-bottom:10px;", sprintf("Value basis: %s", basis)),
        layout_columns(
          col_widths = c(6, 6),
          tags$div(
        ...
        tags$hr(style = "border-color:#1a1a2a;"),
        tags$div(style = "font-weight:700; color:#d0d0d0;", "Impact by pick type and round"),
        breakout_tbl(impact_tbl)
      )
    )
  })
  ...
  pick_movers_table <- function(value_mode, year_filter, round_filter) {
    ...
    rows <- rows %>%
      mutate(
        ...
        Pick = sprintf("%s %s", .data$year, .data$short_label),
        Current = round(.data$cur_mean, 1),
        `3-2-1` = round(.data$new_mean, 1),
        Delta = round(.data$delta, 1)
      )
    bind_rows(
      rows %>% filter(.data$delta > 0) %>% arrange(desc(.data$delta)) %>% slice_head(n = 10),
      rows %>% filter(.data$delta < 0) %>% arrange(.data$delta) %>% slice_head(n = 10)
    ) %>%
      transmute(Direction, Owner = .data$owner, Round = .data$round, Pick, Current, `3-2-1`, !!paste0("Δ ", unit) := Delta)
  }

  render_pick_movers_dt <- function(value_mode) {
    tbl <- pick_movers_table(value_mode, input$pm_year %||% "All", input$pm_round %||% "All")
    delta_col <- tail(names(tbl), 1)
    datatable(
      tbl,
      rownames = FALSE,
  ...
  output$pm_ev_table <- renderDT({
    render_pick_movers_dt("ev")
  })

  output$pm_outcome_table <- renderDT({
    render_pick_movers_dt("outcome")
  })
  ...
  output$tm_teamA_hdr <- renderText(sprintf("Team 1 — %s", input$tm_teamA))
  output$tm_teamB_hdr <- renderText(sprintf("Team 2 — %s", input$tm_teamB))
  ...
  render_trade_pick_table <- function(ids, side, receiver) {
    if (is.null(ids) || length(ids) == 0) {
    ...
    body_rows <- lapply(seq_len(nrow(rows)), function(i) {
      ...
      out_stat <- trade_pick_stats(sent_display_outcome_value(did, side, receiver))

      tags$tr(
        tags$td(style = "min-width:190px;", tags$strong(r$trade_label)),
        tags$td(style = "width:125px;", if (eligible) selectInput(pid, NULL, choices = protection_choices, selected = input[[pid]] %||% "none", width = "115px") else tags$span(style = "color:#777;", "N/A")),
        tags$td(style = "text-align:center; width:95px;", if (eligible) checkboxInput(sid, NULL, value = isTRUE(input[[sid]])) else tags$input(type = "checkbox", disabled = "disabled")),
        tags$td(style = "text-align:right;", sprintf("%.1f", ev_stat["mean"])),
        tags$td(style = "text-align:right;", sprintf("%.1f", ev_stat["q10"])),
        tags$td(style = "text-align:right;", sprintf("%.1f", ev_stat["q90"])),
        tags$td(style = "text-align:right;", sprintf("%.1f", out_stat["mean"])),
        tags$td(style = "text-align:right;", sprintf("%.1f", out_stat["q10"])),
        tags$td(style = "text-align:right;", sprintf("%.1f", out_stat["q90"]))
      )
    })

    total_ev <- trade_pick_stats(side_out_ev_value(ids, side, receiver))
    total_out <- trade_pick_stats(side_outcome_value(ids, side, receiver))
    ...
    tags$div(class = "tm-pick-table",
      tags$table(
        tags$thead(tags$tr(
          ...
          tags$th(style = "text-align:right;", "EPV 10%"),
          tags$th(style = "text-align:right;", "EPV 90%"),
          tags$th(style = "text-align:right;", "4yr WS"),
          tags$th(style = "text-align:right;", "4yr 10%"),
          tags$th(style = "text-align:right;", "4yr 90%")
        )),
        tags$tbody(
          body_rows,
          tags$tr(class = "tm-total-row",
            ...
            tags$td(""),
            tags$td(""),
            tags$td(style = "text-align:right;", sprintf("%.1f", total_ev["mean"])),
            tags$td(style = "text-align:right;", sprintf("%.1f", total_ev["q10"])),
            tags$td(style = "text-align:right;", sprintf("%.1f", total_ev["q90"])),
            tags$td(style = "text-align:right;", sprintf("%.1f", total_out["mean"])),
            tags$td(style = "text-align:right;", sprintf("%.1f", total_out["q10"])),
            tags$td(style = "text-align:right;", sprintf("%.1f", total_out["q90"]))
          )
        )
      )
    )
  ...
  output$tm_picksA_table <- renderUI({
    render_trade_pick_table(input$tm_picksA, "A", receiver = input$tm_teamB)
  })

  output$tm_picksB_table <- renderUI({
    render_trade_pick_table(input$tm_picksB, "B", receiver = input$tm_teamA)
  })
  ...
  output$tm_verdict <- renderUI({
    ...
    signed_metric_card <- function(mean_to_A, q05_to_A, q95_to_A, label) {
      ...
      tags$div(style = paste0(
        "flex:1; min-width:220px; padding:15px; border:1px solid #1a1a2a; ",
        "border-radius:10px; border-left:4px solid #10b981; background:rgba(15,15,26,0.72);"),
        tags$div(style = "font-size:29px; line-height:1.1; font-weight:850; color:#10b981;",
                 sprintf("%+.1f %s to %s", shown_mean, label, to_team)),
        tags$div(style = "font-size:13px; color:#aaa; margin-top:6px;",
    ...
    prob_card <- function(label, p, col) {
      tags$div(style = sprintf(
        "flex:1; min-width:145px; padding:11px 13px; border:1px solid #1a1a2a; border-radius:8px; border-left:3px solid %s;", col),
    ...
    best_outcome_card <- function() {
      tags$div(style = sprintf(
        paste0(
          "min-width:260px; padding:15px; border:1px solid #1a1a2a; border-radius:10px; ",
          "border-left:4px solid %s; background:rgba(15,15,26,0.72); display:flex; ",
          "flex-direction:column; justify-content:center; min-height:112px; box-sizing:border-box;"
        ), best_col),
        tags$div(style = sprintf("font-size:29px; line-height:1.1; font-weight:850; color:%s;", best_col),
    ...
    tags$div(style = "font-size:13px; color:#bbb; line-height:1.7;",
      tags$div(
        style = "display:grid; grid-template-columns:repeat(3, minmax(240px, 1fr)); gap:12px; align-items:stretch; margin-bottom:12px;",
        signed_metric_card(eA_ev, q05_ev, q95_ev, "EPV"),
        signed_metric_card(eA_outcome, q05_outcome, q95_outcome, "4-yr WS"),
        best_outcome_card()
      ),
      tags$div(style = "display:flex; gap:10px; flex-wrap:wrap; min-width:0;",
