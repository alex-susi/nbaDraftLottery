...
# VERSION NOTE (v27 — landscape/impact consistency, pick-count audit fixes):
...
server <- function(input, output, session) {
  ...
  output$impact_chart <- renderPlotly({
    ...
    df_all <- portfolio_quality_quantity_summary_app(
      ...
      mutate(
        ...
        logo_x = if (identical(x_mode, "delta")) .data$new_x else .data$new_avg_quality,
        logo_y = if (identical(x_mode, "delta")) .data$new_y else .data$new_expected_picks,
        hover_text = sprintf(
          paste0(
            "<b>%s</b><br>",
            "Δ total: %+0.2f %s<br>",
    ...
    if (identical(x_mode, "average")) {
      x_vals_all <- df_all$new_avg_quality
      y_vals_all <- df_all$new_expected_picks
    } else {
    ...
    pad_y <- max(0.5, diff(y_rng) * 0.12)

    p <- plot_ly(source = "impact_plot")

    if (identical(x_mode, "delta")) {
      for (sg in c("Positive", "Negative")) {
        sub <- df %>% filter(.data$sign_group == .env$sg)
        if (nrow(sub) == 0) next
        col <- if (sg == "Positive") "#10b981" else "#ef4444"
        p <- p %>%
          ...
          add_markers(
            ...
            x = ~new_x, y = ~new_y,
            customdata = ~team,
            marker = list(color = col, size = 12, opacity = 0.92, line = list(color = "#0f0f1a", width = 1.5)),
            text = ~hover_text,
            hovertemplate = "%{text}<extra></extra>",
            name = sg,
            showlegend = TRUE
    ...
    # Transparent hit targets for logos. Layout images themselves do not reliably
    # emit hover/click events, so these markers carry the hover/click behavior.
    p <- p %>%
      add_markers(
        data = df,
        x = ~logo_x, y = ~logo_y,
        customdata = ~team,
        marker = list(color = "rgba(255,255,255,0.01)", size = 24, line = list(color = "rgba(255,255,255,0)", width = 0)),
        text = ~hover_text,
        hovertemplate = "%{text}<extra></extra>",
        showlegend = FALSE,
        opacity = 0.01
    ...
    logo_images <- team_logo_layout_images_app(
      ...
      x_col = "logo_x",
      y_col = "logo_y",
      sizex = max(0.18, diff(x_rng) * 0.052),
      sizey = max(1.10, diff(y_rng) * 0.080),
      opacity = 0.98
    )
    ...
    x_tickformat <- if (identical(x_mode, "delta")) ".2f" else ".1f"

    p %>%
      plotly_dark(
        xaxis = list(title = list(text = x_title, font = list(size = 15)),
                     tickformat = x_tickformat, tickfont = list(size = 12), range = c(x_rng[1] - pad_x, x_rng[2] + pad_x)),
  ...
  output$wl_dumbbell <- renderPlotly({
    ...
    tier_tbl <- summary_for_value_mode(mode) %>%
      select(team, tier)

    base <- portfolio_quality_quantity_summary_app(mode) %>%
      transmute(
        team,
      ...
      left_join(tier_tbl, by = "team")

    draws <- team_draw_summary_for_table_app(mode, display = TRUE) %>%
    ...
    # Current team logos are drawn as Plotly layout images, with transparent
    # markers underneath for hover. 3-2-1 remains the colored endpoint dot.
    p <- p %>%
      add_markers(
        x = df$current_mean, y = df$y,
        marker = list(color = "rgba(255,255,255,0.01)", size = 12,
    ...
    current_logo_images <- team_logo_layout_images_app(
      df,
      x_col = "current_mean",
      y_col = "y",
      sizex = max(1.1, diff(range(c(df$current_mean, df$new_mean), na.rm = TRUE)) * 0.016),
      sizey = 0.88,
      opacity = 0.98
    )
  ...
  output$team_detail <- renderUI({
    ...
    impact_tbl <- pick_internal_breakout_rows_app(mode) %>%
      ...
      mutate(
        expected_pick_count = pmax(coalesce(.data$cur_convey_prob, 0), coalesce(.data$new_convey_prob, 0))
      ) %>%
