...
PICK_MODEL_R2_HURDLE_PATH     <- c("02_models/pick_play_r2_v5_right_skew_pick_rep.stan", "pick_play_r2_v5_right_skew_pick_rep.stan")
...
pick_fit_r2_hurdle <- sample_pick_stan_model(
  ...
  seed       = 202632,
  label      = "round2_right_skew_hurdle_ws / exact_pick_rep_draws",
  adapt_delta = 0.97,
  max_treedepth = 12
)
pick_fit_r2_hurdle$cmdstan_diagnose()
# Hard stop if the old symmetric Student-t R2 model was accidentally fit from a
# stale 02_models/pick_play_r2_v3.stan or a cached project file. The right-skew
# model must export ws4_pick_rep[1:30], one posterior predictive outcome draw
# for each exact second-round slot. Row-level ws4_rep[1:N] is not a substitute.
r2_draw_variables <- posterior::variables(pick_fit_r2_hurdle$draws())
if (!any(stringr::str_detect(r2_draw_variables, "^ws4_pick_rep\\["))) {
  stop(
    paste(
      "The fitted Round-2 Stan model does not contain ws4_pick_rep[1:30].",
      "You are almost certainly fitting an older pick_play_r2 model.",
      "Copy pick_play_r2_v5_right_skew_pick_rep.stan into 02_models/",
      "and make sure PICK_MODEL_R2_HURDLE_PATH points to that v5 filename.",
      "Do not use ws4_rep here: ws4_rep is row-level PPC output with N columns,",
      "while ws4_pick_rep is exact-pick output with 30 columns.",
      sep = "\n"
    ),
    call. = FALSE
  )
...
pick2_draws <- pick_fit_r2_hurdle$draws(
  variables = c(
    ...
    "logit_upside_prob", "upside_prob", "upside_log_shift", "upside_sigma_mult"
  ),
  format = "df"
) %>% as_tibble()
...
#   * A global rare-upside component captures star outcomes without letting one
#     exact pick slot overfit Jokic / Draymond / Ginobili-style outliers.
...
print(pick_fit_r2_hurdle$summary(c(
  "logit_play_31", "delta_logit_play", "tau_logit_play_rw",
  "log_cond_mean_ws_31", "delta_log_cond_mean_ws", "tau_log_cond_mean_ws_rw",
  "log_sigma_ws_31", "tau_log_sigma_ws_rw",
  "logit_upside_prob", "upside_prob", "upside_log_shift", "upside_sigma_mult"
...
pick2_core_vars <- c(
  ...
  "log_cond_mean_ws_31", "delta_log_cond_mean_ws", "tau_log_cond_mean_ws_rw",
  "log_sigma_ws_31", "tau_log_sigma_ws_rw",
  "logit_upside_prob", "upside_log_shift", "upside_sigma_mult"
)
