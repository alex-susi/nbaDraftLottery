################################################################################
# NBA 3-2-1 Lottery Reform — Shiny Dashboard (v31)
...
server <- function(input, output, session) {
    ...
    df <- portfolio_quality_quantity_summary_app(
      ...
      mutate(
        ...
        hover_text = sprintf(
          ...
          .data$team, .data$current_mean, .data$new_mean, .data$delta_value, .data$lo, .data$hi, .data$n_picks
        ),
      ...
      tags$table(style = "width:100%; border-collapse:collapse; font-size:11px; table-layout:fixed;",
        ...
        tags$tbody(lapply(seq_len(nrow(tbl)), function(i) {
          ...
          tags$tr(
            ...
            tags$td(style = sprintf("padding:4px; text-align:right; color:%s;", ifelse(is.na(r$delta_pct) || r$delta_pct >= 0, "#10b981", "#ef4444")), pct_txt)
          )
        }))
      )
    }
  ...
  output$sp_headline <- renderUI({
    ...
    pct_box <- function(cur, new) {
      ...
      col <- ifelse(is.na(pct) || pct >= 0, "#10b981", "#ef4444")
      tags$div(style = sprintf(
        "flex:1; padding:12px; border:1px solid #1a1a2a; border-radius:8px; border-left:3px solid %s;", col),
        tags$div(style = "font-size:11px; color:#888;", "Δ EPV %"),
        tags$div(style = sprintf("font-size:26px; font-weight:700; color:%s;", col),
  ...
  pick_movers_table <- function(value_mode, year_filter, round_filter, team_filter = "All") {
    ...
    if (is.null(rows) || nrow(rows) == 0L) {
      return(tibble(
        ...
        Round = integer(), Pick = character(), Current = numeric(), `3-2-1` = numeric(),
        !!delta_nm := numeric()
      ))
    }
    ...
    rows %>%
      ...
      transmute(Direction, Owner = .data$owner, Year = as.integer(.data$year),
                Round = .data$round, Pick, Current, `3-2-1`, !!delta_nm := Delta)
  }
  ...
  pick_movers_ui_table <- function(value_mode) {
    tryCatch({
      ...
      table_id <- paste0("pm_table_", value_mode)

      tags$div(class = "pm-simple-wrap",
        tags$table(id = table_id, class = "pm-simple-table",
          tags$thead(tags$tr(lapply(seq_along(col_names), function(k) {
            nm <- col_names[[k]]
            cls <- paste(c(if (nm %in% right_cols) "num" else NULL, "pm-sortable-th"), collapse = " ")
            tags$th(class = cls, `data-table-id` = table_id, `data-sort-col` = k - 1L, onclick = sprintf("pmSortTable('%s', %d)", table_id, k - 1L), nm)
          }))),
          tags$tbody(lapply(seq_len(nrow(tbl)), function(i) {
            r <- tbl[i, , drop = FALSE]
            tags$tr(
              lapply(col_names, function(nm) {
                ...
                tags$td(class = cls, style = style, `data-sort` = as.character(sort_val), as.character(val))
              })
            )
          }))
        )
  ...
  trade_density_plot <- function(x, teamA, teamB, x_title, hover_label = "Net") {
    ...
    fillA <- hex_to_rgba_app(team_secondary_color_app(teamA), 0.32)
    colB <- team_primary_color_app(teamB)
    fillB <- hex_to_rgba_app(team_secondary_color_app(teamB), 0.32)
    ...
    if (nrow(left_df) > 0) {
      p <- p %>% add_lines(data = left_df, x = ~x, y = ~density,
                           ...
                           line = list(color = colB, width = 2.5),
                           fillcolor = fillB,
    ...
    if (nrow(right_df) > 0) {
      p <- p %>% add_lines(data = right_df, x = ~x, y = ~density,
                           ...
                           line = list(color = colA, width = 2.5),
                           fillcolor = fillA,
                           hovertemplate = hover_txt)
    }
    p %>% layout(
      paper_bgcolor = "rgba(0,0,0,0)", plot_bgcolor = "#0f0f1a",
      font = list(family = "IBM Plex Mono", color = "#999"),
      xaxis = list(title = x_title, gridcolor = "#1a1a2a", zerolinecolor = "#f59e0b"),
      yaxis = list(title = "Density", gridcolor = "#1a1a2a"),
      showlegend = FALSE,
