...
ui <- page_navbar(
  ...
  tags$head(tags$style(HTML("
    ...
    body:has(#team_detail .team-detail-card) .full-table-card {
      flex: 0 0 auto;
    }
    ...
    body:has(#team_detail .team-detail-card) #full_table_wrapper .dataTables_scrollBody {
...
server <- function(input, output, session) {
  ...
  output$impact_chart <- renderPlotly({
    ...
    logo_images <- team_logo_layout_images_app(
      df,
      x_col = "cur_avg_quality",
      y_col = "cur_expected_picks",
      sizex = max(0.16, diff(x_rng) * 0.040),
      sizey = max(1.00, diff(y_rng) * 0.065),
      opacity = 0.96
    )
  ...
  output$full_table <- renderDT({
    ...
    datatable(
      ...
      options = list(pageLength = 30, paging = FALSE, dom = "t", ordering = TRUE,
                     scrollY = scroll_y, scrollCollapse = FALSE, scrollX = TRUE,
  ...
  output$team_detail <- renderUI({
    tryCatch({
      ...
      if (is.null(sel) || length(sel) == 0) {
        clicked <- input$full_table_cell_clicked
        if (!is.null(clicked) && !is.null(clicked$row) && !is.na(clicked$row)) {
          sel <- clicked$row
        }
      }
      ...
      source_tbl <- full_table_source() %>% arrange(.data$wins)
      sel_idx <- suppressWarnings(as.integer(sel[1]))
      if (is.na(sel_idx) || sel_idx < 1L || sel_idx > nrow(source_tbl)) return(NULL)

      t <- source_tbl %>% slice(sel_idx)
      tm <- as.character(t$team[[1]])
      if (is.na(tm) || !nzchar(tm)) return(NULL)

      pick_rows <- pick_impact_rows_app(mode) %>%
        filter(.data$owner == .env$tm) %>%
      ...
      html_tbl <- function(tbl, empty_msg = "No picks") {
        if (is.null(tbl) || nrow(tbl) == 0) return(tags$div(style = "color:#777; font-size:11px;", empty_msg))
      ...
      card(class = "mt-2 team-detail-card",
        ...
        card_body(style = "font-size:12px; color:#bbb; line-height:1.6; overflow:visible; max-height:none;",
      ...
      card(class = "mt-2 team-detail-card",
        card_header(style = "border-left:3px solid #ef4444;", "Team detail error"),
        card_body(tags$div(style = "color:#ef4444; font-size:12px;", conditionMessage(e)))
      )
    })
  })
