...
#
# VERSION NOTE (v64 — 30-rank team-strength Markov compatibility):
#   - Methodology tab transition display now reflects tier aggregation derived
#     from the production 30-rank smoothed-softmax Markov model.
#   - Validation panel recognizes team_strength_v2 rank-surface diagnostics.
#
  ...
  nav_panel(
    ...
    div(class = "markov-curve-page",
      ...
      layout_columns(
        col_widths = c(6, 6),
        card(
          card_header("Rank-Model Tier Transition Probabilities"),
          plotlyOutput("trans_heatmap", height = "360px")
        ),
        card(
          card_header("Rank-Model Tier State Diagram"),
  ...
  output$validation_panel <- renderUI({
    ...
    fallback_tbl <- bind_rows(
      ...
      tibble(
        ...
        metric = if (!is.null(mk$state_type) && identical(mk$state_type, "30-rank smoothed softmax")) {
          sprintf("%s rank transitions over %s seasons; row-sum error %s; rank mixing %s seasons; tier mixing %s seasons",
                  ifelse(is.null(mk$n_transitions), "—", format(mk$n_transitions, big.mark = ",")),
                  ifelse(is.null(mk$n_seasons), "—", mk$n_seasons),
                  fmt_metric(mk$row_sum_error, 8),
                  fmt_metric(mk$mixing_time, 1),
                  fmt_metric(mk$tier_mixing_time, 1))
        } else {
          sprintf("%s transitions over %s seasons; Stan vs closed-form max diff %s; mixing time %s seasons",
                  ifelse(is.null(mk$n_transitions), "—", format(mk$n_transitions, big.mark = ",")),
                  ifelse(is.null(mk$n_seasons), "—", mk$n_seasons),
                  fmt_metric(mk$max_abs_diff, 4),
                  fmt_metric(mk$mixing_time, 1))
        },
        pass = if (!is.null(mk$state_type) && identical(mk$state_type, "30-rank smoothed softmax")) {
          ifelse(is.null(mk$row_sum_error), NA, mk$row_sum_error < 1e-8)
        } else {
          ifelse(is.null(mk$max_abs_diff), NA, mk$max_abs_diff < 0.01)
        }
      ),
      tibble(
