...
OUT_DIR <- file.path(PROJECT_DIR, "stan_validation")
...
# The formula below assumes:
#   war_obs[n] ~ normal(alpha / pick[n]^beta + gamma,
#                      sqrt(war_se[n]^2 + (sigma_base + sigma_slope*(pick[n]-1))^2))
...
needed_objects <- c(
  ...
  "pick_stan_data", "pick_slot_data", "counts_mat", "alpha_prior",
...
pick_core_vars <- c("alpha", "beta", "gamma", "sigma_base", "sigma_slope")
...
get_or_generate_pick_gq <- function() {
  if (has_stan_variable(pick_fit, "log_lik") && has_stan_variable(pick_fit, "war_pred")) {
    return(pick_fit)
  }

  if (!GENERATE_LOG_LIK_IF_MISSING) {
    stop("pick_fit does not contain log_lik; set GENERATE_LOG_LIK_IF_MISSING <- TRUE.", call. = FALSE)
  }

  gq_file <- file.path(OUT_DIR, "pick_value_validation_gq.stan")
  writeLines(write_pick_gq_stan(), gq_file)
  gq_model <- cmdstan_model(gq_file)
...
write_pick_gq_stan <- function() {
  paste(
    ...
    "    real sigma_slot = sigma_base + sigma_slope * (pick[n] - 1);",
    "    real obs_sigma = sqrt(square(war_se[n]) + square(sigma_slot));",
    "    log_lik[n] = normal_lpdf(war_obs[n] | mu, obs_sigma);",
    "    war_pred[n] = normal_rng(mu, obs_sigma);",
    "  }",
    "}",
    sep = "\n"
  )
...
war_pred <- array_draws_matrix(pick_gq_fit, "war_pred")
war_obs <- as.numeric(pick_stan_data$war_obs)
pick_obs <- as.numeric(pick_stan_data$pick)

# bayesplot needs a manageable number of replicated draws.
ppc_draw_ids <- sort(sample(seq_len(nrow(war_pred)), size = min(250, nrow(war_pred))))
yrep_pick <- war_pred[ppc_draw_ids, , drop = FALSE]
...
p_pick_intervals <- bayesplot::ppc_intervals(
  ...
  ggplot2::labs(
    title = "Pick-value model PPC: observed slot means vs replicated intervals",
    x = "Draft pick",
...
p_pick_dens <- bayesplot::ppc_dens_overlay(war_obs, yrep_pick[seq_len(min(50, nrow(yrep_pick))), , drop = FALSE]) +
  ggplot2::labs(title = "Pick-value model PPC: distribution overlay")
...
p_pick_stat_mean <- bayesplot::ppc_stat(war_obs, yrep_pick, stat = "mean") +
  ggplot2::labs(title = "Pick-value PPC: mean")
...
p_pick_stat_sd <- bayesplot::ppc_stat(war_obs, yrep_pick, stat = "sd") +
  ggplot2::labs(title = "Pick-value PPC: standard deviation")
save_plot_safe(p_pick_stat_sd, file.path(OUT_DIR, "pick_value_ppc_sd.png"), width = 8, height = 5)

pick_ppc_summary <- tibble(
  pick = pick_obs,
  observed = war_obs,
  pred_mean = colMeans(war_pred),
  pred_q05 = apply(war_pred, 2, quantile, probs = 0.05),
  pred_q50 = apply(war_pred, 2, quantile, probs = 0.50),
  pred_q95 = apply(war_pred, 2, quantile, probs = 0.95),
  covered_90 = observed >= pred_q05 & observed <= pred_q95,
  z_resid = (observed - pred_mean) / apply(war_pred, 2, sd)
)
write_csv_safe(pick_ppc_summary, file.path(OUT_DIR, "pick_value_ppc_summary.csv"))

cat(sprintf("Pick-value 90%% PPC interval coverage: %.1f%%\n", 100 * mean(pick_ppc_summary$covered_90)))
...
# priors inside pick_value.stan. Because nba_lottery.R only compiles the external
# Stan file, this script uses a reasonable prior-like generator. If you change the
# Stan priors, update this generator to match them exactly.

draw_pick_truth <- function() {
  # Return a named numeric vector, not a tibble. This keeps truth[[v]] stable
  # inside purrr::map_dfr() and avoids tibble/list-column edge cases.
  c(
    alpha = rlnorm(1, log(25), 0.35),
    beta = rlnorm(1, log(0.85), 0.25),
    gamma = rnorm(1, 0, 2),
    sigma_base = rlnorm(1, log(3.5), 0.35),
    sigma_slope = rlnorm(1, log(0.04), 0.45)
  )
}

simulate_pick_sbc_data <- function(template_data, truth) {
  truth <- as.numeric(truth)
  names(truth) <- names(draw_pick_truth())
  
  pick <- as.numeric(template_data$pick)
  war_se <- as.numeric(template_data$war_se)
  
  mu <- truth[["alpha"]] / (pick ^ truth[["beta"]]) + truth[["gamma"]]
  sigma_slot <- truth[["sigma_base"]] + truth[["sigma_slope"]] * (pick - 1)
  obs_sigma <- sqrt(war_se^2 + sigma_slot^2)
  
  list(
    N = template_data$N,
    pick = pick,
    war_obs = rnorm(length(pick), mu, obs_sigma),
    war_se = war_se
  )
}
