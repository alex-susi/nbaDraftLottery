################################################################################
# NBA 3-2-1 Lottery Reform — Shiny Dashboard (v41)
# VERSION NOTE (v41 — About landing tab, combined Pick Landscape, methodology diagnostics, display fixes):
...
apply_logo_collision_offsets_app <- function(df, x_col = "x", y_col = "y") {
  ...
  x_tol <- max(0.28, diff(xr) * 0.090)
  y_tol <- max(0.38, diff(yr) * 0.090)
  x_radius <- max(0.14, diff(xr) * 0.026)
  y_radius <- max(0.20, diff(yr) * 0.040)
  ...
  df %>%
    ...
    mutate(
      ...
      .angle_logo = 2 * pi * (.data$.cluster_i_logo - 1) / pmax(.data$.cluster_n_logo, 1),
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        selectInput("impact_view", "View",
          ...
          selected = "scatter"),
        conditionalPanel(
          condition = "input.impact_view == 'scatter'",
  ...
  nav_panel(
    ...
    div(class = "pm-page",
      tags$style(HTML("
        ...
        .pm-page table.dataTable td:nth-child(5), .pm-page table.dataTable th:nth-child(5) { white-space: normal; overflow-wrap: anywhere; }
        .pm-page .card,
        .pm-page .bslib-card,
        .pm-page .card-body,
...
server <- function(input, output, session) {
  ...
  observeEvent(input$pm_clear, {
    ...
    updateSelectizeInput(session, "pm_team", selected = "All", server = TRUE)
  })
  ...
  output$impact_chart <- renderPlotly({
    if (identical(input$impact_view %||% "scatter", "leaderboard")) {
      ...
      logo_images <- team_logo_layout_images_app(
        ...
        x_col = "x", y_col = "y",
        sizex = max(2.6, diff(range(c(df$current_mean, df$new_mean), na.rm = TRUE)) * 0.024),
        sizey = 0.84,
        opacity = 0.98
      )
  ...
  output$validation_panel <- renderUI({
    ...
    table_tag <- tags$table(
      ...
      tags$thead(tags$tr(
        tags$th(style = "text-align:left; color:#cfd2dc; padding:7px; width:22%; border-bottom:1px solid #1a1a2a;", "Model"),
        tags$th(style = "text-align:left; color:#cfd2dc; padding:7px; width:20%; border-bottom:1px solid #1a1a2a;", "Check"),
        tags$th(style = "text-align:left; color:#cfd2dc; padding:7px; width:48%; border-bottom:1px solid #1a1a2a;", "Metric"),
        tags$th(style = "text-align:center; color:#cfd2dc; padding:7px; width:10%; border-bottom:1px solid #1a1a2a;", "Status")
      )),
    ...
    tags$div(style = "font-size:12px; color:#bbb; line-height:1.65;",
      tags$p(style = "margin-bottom:10px; color:#cfd2dc;",
        tags$strong("Why these checks matter: "),
        "R-hat and effective sample size show whether the posterior simulation mixed reliably; NUTS diagnostics check that the Stan sampler did not hit pathological geometry; posterior predictive checks ask whether simulated draft outcomes look like real draft outcomes; PSIS-LOO/Pareto-k checks out-of-sample reliability and whether a few outliers dominate the fit; SBC checks whether the Bayesian uncertainty is calibrated under repeated simulated data. Passing these checks is what makes the pick curves, Markov transitions, and EPV comparisons defensible rather than just visually plausible."
      ),
      table_tag,
  ...
  output$pm_ev_table <- renderDT({
    ...
    datatable(
      ...
      options = list(
        ...
        autoWidth = FALSE,
        columnDefs = list(
          list(width = "88px", targets = 0),
          list(width = "92px", targets = 1),
          list(width = "66px", targets = 2),
          list(width = "66px", targets = 3),
          list(width = "360px", targets = 4),
          list(width = "86px", targets = 5),
          list(width = "86px", targets = 6),
          list(width = "86px", targets = 7),
          list(width = "92px", targets = 8),
