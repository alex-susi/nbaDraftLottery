...
# VERSION NOTE (v28 — EPV-only landscape/impact/table, display-asset audit fixes):
...
split_ranked_display_assets_app <- function() {
  ...
  for (i in seq_len(nrow(split_rows))) {
    ...
    row1$member_n <- 1L
    row2$member_n <- 1L
    row1$label <- stringr::str_squish(paste(rank1, "of", r$member_original_teams, "to", r$owner))
    row2$label <- stringr::str_squish(paste(rank2, "of", r$member_original_teams, "to", r$owner))
...
ui <- page_navbar(
  ...
  nav_panel(
    ...
    layout_sidebar(
      sidebar = sidebar(
        width = 260,
        selectInput("wl_year", "Draft year",
          choices = c("All", sort(unique(pick_display_assets$year))), selected = "All"),
        selectInput("wl_round", "Round",
          choices = c("All" = "All", "Round 1" = "1", "Round 2" = "2"), selected = "All"),
        ...
        actionButton("wl_clear", "Clear filters", class = "btn btn-outline-light btn-sm"),
        tags$p(class = "mono", style = "font-size:11px; color:#888; line-height:1.6; margin-top:12px;",
          "Each row compares current portfolio EPV to 3-2-1 EPV. Team logos mark current value; colored dots mark 3-2-1 value; the horizontal band is the 10th-90th percentile of the 3-2-1 value implied by the rule-change distribution.")
      ),
      card(
      ...
      layout_sidebar(
        sidebar = sidebar(
          ...
          selectInput("pm_round", "Round",
            choices = c("All" = "All", "Round 1" = "1", "Round 2" = "2"), selected = "All"),
          selectInput("pm_team", "Team",
            choices = c("All", sort(unique(pick_display_assets$owner))), selected = "All"),
  ...
  nav_panel(
    ...
    div(class = "tm-page",
      ...
      div(style = "display:flex; justify-content:flex-end; margin: 4px 0 10px;",
        actionButton("tm_reset", "Reset trade", class = "btn btn-outline-light btn-sm")
      ),
      layout_columns(
        col_widths = c(6, 6),
        gap = "2.5rem",
...
server <- function(input, output, session) {
  ...
  observeEvent(input$pm_clear, {
    ...
    updateSelectInput(session, "pm_team", selected = "All")
  })
  ...
  output$impact_title <- renderText({
    ...
    sprintf("Pick Quality vs Pick Quantity — Expected Pick Value, %s, %s", yr_txt, rnd_txt)
  })
  ...
  output$impact_chart <- renderPlotly({
    ...
    x_rng <- range(df_all$x, na.rm = TRUE)
    y_rng <- range(df_all$y, na.rm = TRUE)
    ...
    # Invisible, clickable/hoverable hit targets under each logo. Add one trace
    # per team so the hover box can use that team's brand color.
    for (i in seq_len(nrow(df))) {
      p <- p %>% add_markers(
        x = df$x[i], y = df$y[i], customdata = df$team[i],
        marker = list(color = "rgba(255,255,255,0.01)", size = 34, line = list(color = "rgba(255,255,255,0)", width = 0)),
        text = df$hover_text[i], hovertemplate = "%{text}<extra></extra>",
        hoverlabel = list(bgcolor = df$primary[i], font = list(color = df$text_col[i])),
    ...
    annotations <- list(
      list(x = x_rng[2] + pad_x * 0.52, y = y_rng[2] + pad_y * 0.48, text = "High quality / high quantity", showarrow = FALSE, font = list(size = 12, color = "#10b981")),
      list(x = x_rng[2] + pad_x * 0.52, y = y_rng[1] - pad_y * 0.28, text = "High quality / low quantity", showarrow = FALSE, font = list(size = 12, color = "#aaa")),
      list(x = x_rng[1] - pad_x * 0.52, y = y_rng[2] + pad_y * 0.48, text = "Low quality / high quantity", showarrow = FALSE, font = list(size = 12, color = "#aaa")),
      list(x = x_rng[1] - pad_x * 0.52, y = y_rng[1] - pad_y * 0.28, text = "Low quality / low quantity", showarrow = FALSE, font = list(size = 12, color = "#ef4444"))
    )
    ...
    p %>%
      plotly_dark(
        ...
        shapes = shapes, annotations = annotations, images = logo_images,
  ...
  output$wl_dumbbell <- renderPlotly({
    ...
    df <- portfolio_quality_quantity_summary_app(
      ...
      mutate(
        ...
        hover_text = sprintf(
          ...
          .data$team, .data$current_mean, .data$new_mean, .data$delta_value, .data$lo, .data$hi
        ),
    ...
    logo_images <- team_logo_layout_images_app(
      ...
      x_col = "x", y_col = "y",
      sizex = max(2.0, diff(range(c(df$current_mean, df$new_mean), na.rm = TRUE)) * 0.018),
      sizey = 0.72,
      opacity = 0.98
    )
    ...
    for (i in seq_len(nrow(df))) {
      ...
      p <- p %>% add_segments(
        x = df$lo[i], xend = df$hi[i], y = df$y[i], yend = df$y[i],
        line = list(color = "rgba(255,255,255,0.18)", width = 6),
        text = df$hover_text[i], hovertemplate = "%{text}<extra></extra>",
        hoverlabel = list(bgcolor = df$col[i], font = list(color = "white")),
        showlegend = FALSE
      )
      # Connector from current logo to 3-2-1 dot.
      p <- p %>% add_segments(
        x = df$current_mean[i], xend = df$new_mean[i], y = df$y[i], yend = df$y[i],
        line = list(color = df$col[i], width = 2), opacity = 0.55,
        text = df$hover_text[i], hovertemplate = "%{text}<extra></extra>",
        hoverlabel = list(bgcolor = df$col[i], font = list(color = "white")),
        showlegend = FALSE
      )
      ...
      p <- p %>% add_markers(
        x = df$current_mean[i], y = df$y[i], customdata = df$team[i],
        marker = list(color = "rgba(255,255,255,0.01)", size = 20, line = list(color = "rgba(255,255,255,0)", width = 0)),
        text = df$hover_text[i], hovertemplate = "%{text}<extra></extra>",
        hoverlabel = list(bgcolor = df$col[i], font = list(color = "white")),
        showlegend = FALSE, opacity = 0.01
      )
      # 3-2-1 endpoint.
      p <- p %>% add_markers(
        x = df$new_mean[i], y = df$y[i], customdata = df$team[i],
        marker = list(color = df$col[i], size = 9, opacity = 0.95, line = list(color = "#0a0a14", width = 1.3)),
        text = df$hover_text[i], hovertemplate = "%{text}<extra></extra>",
        hoverlabel = list(bgcolor = df$col[i], font = list(color = "white")),
        showlegend = FALSE
      )
    ...
    p %>%
      plotly_dark(
        xaxis = list(title = "Portfolio value (EPV)", range = c(rng[1] - pad, rng[2] + pad)),
        yaxis = list(title = "", tickmode = "array", tickvals = df$y, ticktext = as.character(df$team), autorange = FALSE),
        images = logo_images,
  ...
  output$full_table <- renderDT({
    ...
    tbl <- base %>%
      transmute(
        ...
        `Current EPV` = round(.data$current_mean, 1),
        `3-2-1 EPV` = round(.data$new_mean, 1),
        `Δ EPV` = round(.data$delta_value, 1),
        `Δ%` = delta_pct_html
      )
    ...
    datatable(
      ...
      formatStyle("Δ EPV", color = styleInterval(0, c("#ef4444", "#10b981")))
  }, server = FALSE)
  ...
  output$team_detail <- renderUI({
    ...
    impact_tbl <- pick_impact_rows_app(mode) %>%
      ...
      complete(
        ...
        round = c(1L, 2L),
        fill = list(n_picks = 0, current = 0, new = 0, delta = 0)
      ) %>%
    ...
    html_tbl <- function(tbl, empty_msg = "No picks") {
      ...
      tags$table(style = "width:100%; border-collapse:collapse; font-size:11px; table-layout:fixed;",
        tags$thead(tags$tr(
          tags$th(style = "text-align:left; color:#aaa; padding:4px; width:52%;", "Pick"),
          tags$th(style = "text-align:right; color:#aaa; padding:4px; width:16%;", paste0("Δ ", unit)),
          tags$th(style = "text-align:right; color:#aaa; padding:4px; width:16%;", "Cur"),
          tags$th(style = "text-align:right; color:#aaa; padding:4px; width:16%;", "3-2-1")
        )),
    ...
    breakout_tbl_round <- function(tbl, round_no) {
      ...
      tags$table(style = "width:100%; border-collapse:collapse; font-size:11px; margin-top:5px; table-layout:fixed;",
        tags$thead(tags$tr(
          tags$th(style = "text-align:left; color:#aaa; padding:4px; width:42%;", "Bucket"),
          tags$th(style = "text-align:right; color:#aaa; padding:4px; width:16%;", "# Picks"),
          tags$th(style = "text-align:right; color:#aaa; padding:4px; width:14%;", "Cur"),
          tags$th(style = "text-align:right; color:#aaa; padding:4px; width:14%;", "3-2-1"),
          tags$th(style = "text-align:right; color:#aaa; padding:4px; width:14%;", paste0("Δ ", unit))
        )),
        tags$tbody(lapply(seq_len(nrow(tbl)), function(i) {
          r <- tbl[i, ]
          tags$tr(
    ...
    card(class = "team-detail-card",
      ...
      card_body(style = "font-size:12px; color:#bbb; line-height:1.45; overflow-x:hidden; padding-top:10px;",
        layout_columns(
          ...
          tags$div(
            tags$div(style = "font-weight:700; color:#ef4444; margin:10px 0 4px;", "Most negative pick impacts"),
            html_tbl(neg_tbl, "No negative pick impacts")
          )
        ),
        tags$hr(style = "border-color:#1a1a2a; margin:12px 0 8px 0;"),
        tags$div(style = "font-weight:700; color:#d0d0d0; margin-top:2px;", "Impact by pick type and round"),
        layout_columns(
          col_widths = c(12),
          tags$div(tags$div(style = "font-weight:700; color:#aaa; margin-top:6px;", "Round 1"), breakout_tbl_round(impact_tbl, 1L)),
          tags$div(tags$div(style = "font-weight:700; color:#aaa; margin-top:8px;", "Round 2"), breakout_tbl_round(impact_tbl, 2L))
        )
      )
    )
  })
  ...
  observeEvent(input$sp_year, {
    ...
    updateSelectInput(session, "sp_team", choices = all_team_abbr, selected = cur_team)
  }, ignoreNULL = FALSE)
  ...
  pick_movers_ui_table <- function(value_mode) {
    tryCatch({
      tbl <- pick_movers_table(value_mode, input$pm_year %||% "All", input$pm_round %||% "All", input$pm_team %||% "All")
      unit <- value_mode_unit(value_mode)
