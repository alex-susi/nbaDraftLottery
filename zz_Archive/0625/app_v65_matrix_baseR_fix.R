...
# VERSION NOTE (v47 — refine Pick Landscape logo layering/collisions and Single Pick details):
...
#
# VERSION NOTE (v65 — display-asset de-duplication + rank methodology view):
...
#   - Adds the posterior-mean 30-rank transition heatmap to Methodology while
#     retaining the existing five-tier aggregation and state diagram.
#   - Uses a class-stripped numeric matrix and base-R raster rendering for the
#     30-rank heatmap, bypassing Plotly/ggplot text serialization entirely.
#
...
#     which is the reliable mode for HTML-cell tables of this size. Team Summaries
...
#     3-2-1 value).
#   - Team Summaries: replaced the z-score "Signal" column with a clearer directional
...
#   - New Winners & Losers dumbbell tab; Team Summaries inline interval-bar + signal.
#   - Team Summaries single-scroll wrapper; Trade Machine column widening + gap.
#
...
message("Launching app_v65_matrix_baseR_fix.R (base-R 30-rank heatmap)")
  ...
  nav_panel(
    ...
    div(class = "markov-curve-page",
      ...
      layout_columns(
        col_widths = c(12),
        card(
          card_header("30-Rank Team-Strength Transition Matrix"),
          tags$p(
            style = "color:#9ca3af; font-size:12px; margin:0 14px 4px;",
            "Each cell is the posterior-mean probability of moving from the current-season record rank to the following-season record rank. Rank 01 is the league's worst record and Rank 30 is the best."
          ),
          plotOutput("rank_trans_heatmap", height = "760px")
        )
      ),
      layout_columns(
        col_widths = c(12),
...
server <- function(input, output, session) {
  ...
  # ---- Full 30-rank transition heatmap ----
  output$rank_trans_heatmap <- renderPlot({
    if (is.null(rank_trans_mat) || length(rank_trans_mat) != 900L) {
      plot.new()
      par(bg = "#0f0f1a", fg = "#d1d5db")
      text(
        0.5, 0.5,
        labels = "A valid 30 x 30 rank transition matrix was not found in dashboard_data.rds.",
        col = "#d1d5db", cex = 1.0
      )
      return(invisible(NULL))
    }

    # Strip any posterior/tibble/draws classes and dimname attributes before
    # plotting. Only a plain 30x30 double matrix enters the graphics device.
    m <- matrix(
      suppressWarnings(as.numeric(rank_trans_mat)),
      nrow = 30L,
      ncol = 30L,
      byrow = FALSE
    )
    if (any(!is.finite(m))) {
      stop("rank_transition_matrix contains non-finite values.", call. = FALSE)
    }

    z <- 100 * t(m[30:1, , drop = FALSE])
    z_min <- min(z, na.rm = TRUE)
    z_max <- max(z, na.rm = TRUE)
    if (!is.finite(z_min) || !is.finite(z_max) || z_max <= z_min) {
      z_min <- 0
      z_max <- max(1, z_max)
    }

    heat_cols <- grDevices::colorRampPalette(
      c("#0f0f1a", "#172554", "#4c1d95", "#a78bfa")
    )(160)
    rank_labels <- sprintf("Rank %02d", 1:30)
    ...
    axis(1, at = 1:30, labels = rank_labels, las = 2, tick = FALSE, cex.axis = 0.58, line = -0.2)
    axis(2, at = 1:30, labels = rev(rank_labels), las = 1, tick = FALSE, cex.axis = 0.58, line = -0.2)
    mtext("Following-season rank (Year t + 1)", side = 1, line = 6.7, col = "#d1d5db", cex = 0.9)
    mtext("Current-season rank (Year t)", side = 2, line = 6.5, col = "#d1d5db", cex = 0.9)

    # Draw a base-R probability legend inside the reserved right margin.
    legend_edges <- seq(1, 30, length.out = length(heat_cols) + 1L)
    for (i in seq_along(heat_cols)) {
      rect(31.3, legend_edges[i], 32.2, legend_edges[i + 1L],
           col = heat_cols[i], border = NA)
    }
    legend_values <- pretty(c(z_min, z_max), n = 5)
    legend_values <- legend_values[legend_values >= z_min & legend_values <= z_max]
    legend_y <- 1 + 29 * (legend_values - z_min) / (z_max - z_min)
    text(32.55, legend_y, labels = sprintf("%.1f%%", legend_values),
         adj = 0, col = "#9ca3af", cex = 0.68)
    text(31.75, 30.75, labels = "Probability", col = "#d1d5db", cex = 0.72)
    box(col = "#374151")
  }, bg = "#0f0f1a", res = 120)
