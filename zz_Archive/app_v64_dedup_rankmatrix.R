...
#
# VERSION NOTE (v64 — display-asset de-duplication + rank methodology view):
    ...
    div(class = "markov-curve-page",
      ...
      layout_columns(
        ...
        card(
          ...
          plotlyOutput("rank_trans_heatmap", height = "760px")
        )
      ),
      layout_columns(
        col_widths = c(12),
...
server <- function(input, output, session) {
  ...
  output$rank_trans_heatmap <- renderPlotly({
    validate(need(
      !is.null(rank_trans_mat) && is.matrix(rank_trans_mat) && identical(dim(rank_trans_mat), c(30L, 30L)),
      "The current dashboard_data.rds does not include the 30-rank transition matrix. Rebuild it with the rank-based team-strength model export enabled."
    ))
    ...
    plot_ly(
      ...
      colorbar = list(title = "Probability (%)", thickness = 14),
      ...
      plotly_dark(
        xaxis = list(
          title = list(text = "Following-season rank (Year t + 1)", standoff = 18),
          side = "bottom",
          tickangle = 45,
        ...
        yaxis = list(
          title = list(text = "Current-season rank (Year t)", standoff = 18),
          autorange = "reversed",
          tickfont = list(size = 9),
          automargin = TRUE
        ),
        margin = list(l = 105, r = 90, t = 20, b = 115)
      )
  })
