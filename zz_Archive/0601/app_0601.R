...
server <- function(input, output, session) {
  ...
  # detail panels listing obligations on selected picks
  render_pick_detail <- function(ids) {
    ...
    tags$div(style = "font-size:11px; color:#aaa; line-height:1.6;",
      lapply(seq_len(nrow(rows)), function(i) {
        r <- rows[i, ]
        via <- if (r$is_traded)
          sprintf(" — via %s (orig %s)", r$owner, r$original_team) else ""
        tags$div(
          tags$strong(r$label),
  ...
  # Joint per-sim net value transferred.
  # A sends picksA (loses them) and receives picksB; net to A = B's picks - A's picks.
  trade_draws <- reactive({
    ...
    sumcols <- function(M, ids) {
      if (is.null(ids) || length(ids) == 0) return(rep(0, nrow(M)))
      if (length(ids) == 1) return(M[, ids])
      rowSums(M[, ids, drop = FALSE])
    }
    # value under 3-2-1 (the system going forward); also compute current for ref
    A_out_new <- sumcols(asset_new_draws, idsA)
    B_out_new <- sumcols(asset_new_draws, idsB)
    A_out_cur <- sumcols(asset_cur_draws, idsA)
    B_out_cur <- sumcols(asset_cur_draws, idsB)
    tibble(
      net_to_A_new = B_out_new - A_out_new,   # A gains B's picks, loses its own
      net_to_B_new = A_out_new - B_out_new,
      net_to_A_cur = B_out_cur - A_out_cur
    )
  })
  ...
  output$tm_verdict <- renderUI({
    ...
    tags$div(style = "font-size:12px; color:#bbb; line-height:1.8;",
      ...
      tags$div(style = "color:#888; margin-top:6px;",
        sprintf("A sends %d pick(s); B sends %d pick(s). Values are 4-yr Win Shares under the 3-2-1 system, computed with within-simulation correlation so both sides move together.",
