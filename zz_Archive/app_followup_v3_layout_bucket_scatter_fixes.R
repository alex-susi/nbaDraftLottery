...
portfolio_quality_quantity_summary_app <- function(value_mode, year_filter = "All", round_filter = "All") {
  ...
  purrr::map_dfr(all_summary_teams, function(tm) {
    ...
    # The x-axis is average value per displayed pick entitlement, not value per
    # expected-conveyed pick. Expected pick count remains the y-axis.
  ...
  out %>%
    mutate(
      impact_bucket = {
        ...
        short_txt <- stringr::str_to_lower(as.character(.data$short_label %||% ""))
        txt <- stringr::str_to_lower(paste(.data$group_type %||% "", .data$obligation %||% "", .data$notes %||% "", .data$label %||% "", .data$short_label %||% ""))

        # Classify user-facing labels first. Plain "via TEAM" rows are true
        # incoming outright picks; "own or TEAM via ... swap" rows are swap
        # entitlements even though they also contain the word "via".
        short_is_swap_or_cond <- stringr::str_detect(short_txt, "own or|swap|protect|conditional|complex|favorable|ranked|pool")
        short_is_incoming_outright <- stringr::str_detect(short_txt, "\\bvia\\b") & !short_is_swap_or_cond
        constrained <- stringr::str_detect(txt, "swap|protect|protected|conditional|complex|favorable|ranked|pool")
        simple_outright <- stringr::str_detect(txt, "outright|single_asset|\\bto\\b")

        dplyr::case_when(
          short_is_swap_or_cond | constrained ~ "Swaps / protections",
          short_is_incoming_outright | (!own_team & simple_outright) ~ "Incoming outright picks",
          own_team ~ "Own picks",
          TRUE ~ "Incoming outright picks"
        )
      }
    )
}
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        p(style = "font-size:11px; color:#888; line-height:1.6;",
          ...
          "The x-axis is average value per displayed pick, the y-axis is expected pick count, ",
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        selectInput("pm_round", "Round",
          choices = c("All" = "All", "Round 1" = "1", "Round 2" = "2"), selected = "All"),
        tags$p(style = "font-size:11px; color:#888; line-height:1.6;",
          "Shows all display pick entitlements matching the selected filters, sorted by 3-2-1 impact.")
      ),
      layout_columns(
        col_widths = c(6, 6),
        card(
...
server <- function(input, output, session) {
  ...
  output$impact_chart <- renderPlotly({
      ...
      mutate(
        ...
        hover_text = sprintf(
          paste0(
            ...
            "3-2-1: %.2f %s across %.2f expected picks<br>",
            "Displayed picks in filter: %d<br><br>",
            "Δ Total: %+.2f %s<br>",
            "Δ Avg value / displayed pick: %+.2f<br>",
    ...
    p <- p %>%
      add_markers(
        ...
        text = ~sprintf("<b>%s current</b><br>Avg value / displayed pick: %.2f<br>Expected picks: %.2f<br>Total: %.2f %s",
    ...
    p %>%
      plotly_dark(
        xaxis = list(title = list(text = paste0("Average ", value_mode_unit(input$impact_value_mode), " per displayed pick"), font = list(size = 15)),
  ...
  render_pick_movers_dt <- function(value_mode) {
    ...
    dt <- datatable(
      tbl,
      rownames = FALSE,
      options = list(pageLength = 50, dom = "tip", ordering = TRUE,
    ...
    if (pad_n > 0L) {
      spacer_rows <- lapply(seq_len(pad_n), function(i) {
        tags$tr(class = "tm-spacer-row",
          tags$td(style = "height:42px; padding:6px 0; border-bottom:0;", HTML("&nbsp;")),
          tags$td(style = "height:42px; padding:6px 0; border-bottom:0;", HTML("&nbsp;")),
          tags$td(style = "height:42px; padding:6px 0; border-bottom:0;", HTML("&nbsp;")),
          tags$td(style = "height:42px; padding:6px 0; border-bottom:0;", HTML("&nbsp;"))
        )
