    ...
    div(class = "markov-curve-page",
      ...
      layout_columns(
        ...
        div(class = "curve-stack",
          card(
            card_header("Round 1 Draft Pick Value Curve (4-yr Win Shares)"),
            plotlyOutput("pick_curve_plot_r1", height = "320px")
          ),
          card(
            card_header("Round 2 Draft Pick Value Curve — P(play) Hurdle + Empirical WS EV"),
...
server <- function(input, output, session) {
  ...
  pick_curve_base_plot <- function(pc, x_title, y_title = "4-year Win Shares") {
    pc <- pc %>%
      mutate(
        war_sd = if_else(is.na(.data$war_sd), 0, .data$war_sd),
        ribbon_low = .data$expected_war - .data$war_sd,
        ribbon_high = .data$expected_war + .data$war_sd
      )

    p <- plot_ly(pc, x = ~pick) %>%
      add_ribbons(ymin = ~ribbon_low,
                  ymax = ~ribbon_high,
                  name = "+/- 1 SD",
    ...
    if ("emp_mean" %in% names(pc) && any(is.finite(pc$emp_mean))) {
      p <- p %>%
        add_markers(y = ~emp_mean, name = "Empirical slot mean",
                    ...
                    hovertemplate = "Pick %{x}<br>Empirical mean: %{y:.2f}<extra></extra>")
    }

    p %>%
      add_lines(y = ~expected_war, name = "Posterior mean / EV",
                line = list(color = "#6d28d9", width = 2.5),
                hovertemplate = "Pick %{x}<br>EV: %{y:.2f}<extra></extra>") %>%
      plotly_dark(
        xaxis = list(title = x_title, dtick = 5),
        yaxis = list(title = y_title),
        legend = list(x = 0.50, y = 0.95, font = list(size = 10))
      )
  }
  ...
  output$pick_curve_plot_r1 <- renderPlotly({
    ...
    pick_curve_base_plot(pc, x_title = "Round 1 Draft Pick")
  })
    ...
    if (nrow(pc) == 0) {
      return(plot_ly() %>%
               plotly_dark(
                 xaxis = list(title = "Draft Pick"),
                 yaxis = list(title = "4-year Win Shares"),
                 annotations = list(text = "Round 2 curve unavailable — dashboard_data.rds lacks picks 31-60 / mu_31:mu_60. Rerun nba_lottery_second_round_full_fixed.R", x = 0.5, y = 0.5,
                                    xref = "paper", yref = "paper", showarrow = FALSE)))
    }

    r2_y_title <- "Expected 4-year WS (including zero hurdle)"

    p <- pick_curve_base_plot(
      pc,
      x_title = "Round 2 Draft Pick",
      y_title = r2_y_title
    )

    # The second-round model estimates P(play) in Stan and values played outcomes with an empirical/Bayesian-bootstrap WS distribution. When p_play is available, add it
    # on a secondary axis so the plot shows both expected value and the modeled
    # probability that the pick produces an NBA-minutes outcome.
    if ("p_play" %in% names(pc) && any(is.finite(pc$p_play))) {
      ...
      p <- p %>%
        layout(
          ...
          margin = list(l = 70, r = 150, t = 96, b = 58),
          yaxis = list(
            title = r2_y_title,
            automargin = TRUE,
            gridcolor = "#1a1a2a",
  ...
  output$pick_curve_plot <- renderPlotly({
    pc <- pick_curve
    pick_curve_base_plot(pc, x_title = "Draft Pick")
  })
