...
server <- function(input, output, session) {
  ...
  output$team_detail <- renderUI({
    ...
    card(class = "team-detail-card",
      ...
      card_body(style = "font-size:12px; color:#bbb; line-height:1.45; overflow-x:hidden; padding-top:10px;",
        ...
        tags$hr(style = "border-color:#1a1a2a; margin:6px 0 5px 0;"),
        tags$div(style = "font-weight:700; color:#d0d0d0; margin-top:0; margin-bottom:2px;", "Impact by pick type and round"),
        layout_columns(
          col_widths = c(12),
          gap = "4px",
  ...
  output$sp_obligation <- renderUI({
    ...
    tags$div(style = "font-size:11px; color:#aaa; line-height:1.55;",
      tags$div(style = "margin-bottom:12px;", tags$strong(style = "color:#ffffff; font-size:15px;", "Pick Details")),
      team_detail_row(
        "Current Team:",
        tags$span(class = "sp-team-inline", team_logo_img_app(r$owner, size = 18), tags$span(r$owner))
      ),
      team_detail_row(
        ...
        tagList(lapply(split_abbrs(r$member_original_teams), function(tm) {
          tags$span(class = "sp-team-inline", team_logo_img_app(tm, size = 18), tags$span(tm))
        }))
      ),
