...
ui <- page_navbar(
  ...
  tags$head(tags$style(HTML("
    ...
    .full-table-card .card-body,
    .full-table-card .bslib-card-body,
    .full-table-card [data-card-body] {
      min-height: 0 !important;
      overflow: hidden !important;
    }
    #full_table_wrapper .dataTables_scrollHead {
      overflow: hidden !important;
    }
    #full_table_wrapper .dataTables_scrollBody {
      overflow-y: auto !important;
      height: calc(100vh - 250px) !important;
      max-height: calc(100vh - 250px) !important;
    }
    .full-table-page:has(.team-detail-card) .full-table-card {
      flex: 0 0 auto;
    }
    .full-table-page:has(.team-detail-card) #full_table_wrapper .dataTables_scrollBody {
    ...
    .pick-movers-page .dataTables_scrollBody {
      overflow-y: auto !important;
    }
    .tm-page .tm-pick-table {    .tm-page .tm-pick-table {
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        p(style = "font-size:11px; color:#888; line-height:1.6;",
          ...
          "Toggle the x-axis between average value per pick and change in value per pick. ",
          "Size reflects portfolio materiality, and color reflects the total 3-2-1 impact.",
          br(), br(),
  ...
  nav_panel(
    ...
    layout_sidebar(
      ...
      div(class = "full-table-page",
        card(class = "full-table-card",
    ...
    div(class = "markov-curve-page",
      ...
      layout_columns(
        ...
        card(
          ...
          plotlyOutput("pick_curve_plot_r1", height = "390px")
        ),
        card(
...
server <- function(input, output, session) {
  ...
  output$impact_chart <- renderPlotly({
    ...
    df_all <- portfolio_quality_quantity_summary_app(
      ...
      mutate(
        ...
        size_scale = rescale_numeric_app(.data$size_metric, c(0.85, 1.45)),
        marker_size = 18 + 22 * (.data$size_scale - min(.data$size_scale, na.rm = TRUE)) /
          pmax(diff(range(.data$size_scale, na.rm = TRUE)), 0.001),
        arrow_width = 2.2 + 3.6 * (.data$size_scale - min(.data$size_scale, na.rm = TRUE)) /
          pmax(diff(range(.data$size_scale, na.rm = TRUE)), 0.001),
        hover_text = sprintf(
          paste0(
            "<b>%s</b><br>",
    ...
    # Clicking a logo/team isolates that team. The axis ranges still use the full
    # league so the selected team's movement remains contextual.
    ...
    df <- df %>%
      mutate(
        logo_sizex = pmax(0.10, diff(x_rng) * 0.030 * .data$size_scale),
        logo_sizey = pmax(0.65, diff(y_rng) * 0.050 * .data$size_scale)
      )

    p <- plot_ly(source = "impact_plot")

    # Draw each arrow as its own trace so width can scale by portfolio/value size.
    for (i in seq_len(nrow(df))) {
      row <- df[i, ]
      col <- if (isTRUE(row$delta_total_value >= 0)) "#10b981" else "#ef4444"
      seg_df <- tibble(
        x = c(row$cur_x, row$new_x),
        y = c(row$cur_y, row$new_y),
        team = rep(row$team, 2),
        hover_text = rep(row$hover_text, 2)
      )
      p <- p %>%
        add_trace(
          data = seg_df,
          ...
          y = ~y,
          customdata = ~team,
          line = list(color = col, width = as.numeric(row$arrow_width)),
          opacity = 0.84,
          text = ~hover_text,
          hovertemplate = "%{text}<extra></extra>",
          showlegend = FALSE,
    ...
    # Transparent current-position hit targets make logo hover/click work even
    # though the logos themselves are Plotly layout images.
    p <- p %>%
      add_markers(
        data = df,
        x = ~cur_x, y = ~cur_y,
    ...
    logo_images <- purrr::map(seq_len(nrow(df)), function(i) {
      src <- team_logo_plot_url_app(df$team[i])
      if (length(src) == 0L || is.na(src) || !nzchar(src)) return(NULL)
      list(
        source = src,
        xref = "x", yref = "y",
        x = df$cur_x[i], y = df$cur_y[i],
        xanchor = "center", yanchor = "middle",
        sizex = df$logo_sizex[i], sizey = df$logo_sizey[i],
        sizing = "contain",
        opacity = 0.98,
        layer = "above"
      )
  ...
  output$full_table <- renderDT({
    ...
    tbl <- full_table_source() %>%
      ...
      transmute(
        ...
        `P(+ Impact)` = sprintf("%.1f%%", 100 * coalesce(.data$p_positive, 0)),
    ...
    datatable(
      ...
      options = list(
        ...
        scrollY = "calc(100vh - 250px)", scrollCollapse = FALSE, scrollX = TRUE,
        columnDefs = list(
          list(className = "dt-right", targets = 3:10),
          list(visible = FALSE, searchable = FALSE, targets = 11)
        )
      ),
      callback = htmlwidgets::JS("
        table.on('click', 'tbody tr', function() {
          ...
          if (data && data.length > 0) {
            Shiny.setInputValue('full_table_team_clicked', data[data.length - 1], {priority: 'event'});
          }
        });
  ...
  output$team_detail <- renderUI({
    tm <- input$full_table_team_clicked
    if (is.null(tm) || length(tm) == 0 || !nzchar(as.character(tm))) return(NULL)
    t <- full_table_source() %>% filter(.data$team == as.character(.env$tm)) %>% slice(1)
    ...
    card(class = "mt-2 team-detail-card",
      ...
      card_body(style = "font-size:12px; color:#bbb; line-height:1.5; overflow:visible; max-height:none; padding-top:8px;",
        layout_columns(
          col_widths = c(6, 6),
          tags$div(
  ...
  pick_movers_table <- function(value_mode, year_filter, round_filter) {
    ...
    empty_tbl <- tibble(
      Direction = character(), Owner = character(), Round = integer(),
      Current = numeric(), `3-2-1` = numeric(), !!delta_nm := numeric(), PickInfo = character()
    )
    ...
    rows %>%
      ...
      transmute(Direction, Owner = .data$owner, Round = .data$round, Current, `3-2-1`, !!delta_nm := Delta, PickInfo)
  }
  ...
  render_pick_movers_dt <- function(value_mode) {
    tryCatch({
      ...
      if (is.null(tbl) || nrow(tbl) == 0L) {
        return(datatable(tibble(Message = "No picks match the selected filters."), rownames = FALSE,
                         options = list(dom = "t", paging = FALSE)))
      }

      delta_col <- names(tbl)[ncol(tbl) - 1]
      ...
      dt <- datatable(
        ...
        callback = JS(
          ...
          "    var d = this.data();",
          "    if (d && d.length > 0) {",
          "      var tip = d[d.length - 1];",
          "      $(this.node()).attr('title', tip);",
          "      $('td', this.node()).attr('title', tip);",
          "    }",
          "  });",
          "}"
        ),
        options = list(
          ...
          scrollX = FALSE,
          columnDefs = list(
            list(className = "dt-right", targets = c(3, 4, 5)),
            list(visible = FALSE, searchable = FALSE, targets = 6),
            list(width = "18%", targets = c(0, 1)),
            list(width = "10%", targets = c(2, 3, 4, 5))
          )
        )
      )
      ...
      datatable(tibble(Error = paste("Pick Movers failed:", conditionMessage(e))), rownames = FALSE,
                options = list(dom = "t", paging = FALSE))
    })
  }
  ...
  render_trade_pick_table <- function(ids, side, receiver, pad_to = NULL) {
    ...
    body_rows <- lapply(seq_len(nrow(rows)), function(i) {
      ...
      tags$tr(
        tags$td(style = "width:52%; max-width:0; font-size:15px;", tags$strong(class = "tm-pick-label", r$trade_label)),
