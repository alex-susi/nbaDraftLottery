...
pick_impact_rows_app <- function(value_mode) {
  ...
  out %>%
    mutate(
      impact_bucket = {
        origs <- as.character(.data$member_original_teams %||% "")
        owner_chr <- as.character(.data$owner)
        own_team <- purrr::map2_lgl(origs, owner_chr, function(o, tm) {
          if (is.na(o) || is.na(tm) || !nzchar(o) || !nzchar(tm)) return(FALSE)
          tm %in% split_abbrs(o)
        })
        txt <- stringr::str_to_lower(paste(.data$group_type %||% "", .data$obligation %||% "", .data$notes %||% "", .data$label %||% "", .data$short_label %||% ""))
        constrained <- stringr::str_detect(txt, "swap|protect|protected|conditional|complex|favorable|ranked|pool")
        simple_single <- stringr::str_detect(txt, "single_asset|outright|\\bvia\\b|\\bto\\b")
        dplyr::case_when(
          own_team & !constrained ~ "Own picks",
          !own_team & !constrained & simple_single ~ "Incoming outright picks",
...
server <- function(input, output, session) {
  ...
  output$impact_chart <- renderPlotly({
    ...
    for (sg in c("Positive", "Negative")) {
      ...
      p <- p %>%
        ...
        add_markers(
          ...
          size = ~bubble_size,
          sizes = c(16, 56),
          marker = list(color = col, opacity = 0.90, line = list(color = "#0f0f1a", width = 1.5)),
          text = ~hover_text,
          hovertemplate = "%{text}<extra></extra>",
          name = sg,
          showlegend = TRUE
    ...
    p <- p %>%
      add_markers(
        ...
        marker = list(color = "#94a3b8", size = 9, opacity = 0.52, symbol = "circle-open", line = list(width = 1.5)),
      ...
      add_text(
        ...
        textfont = list(size = 13, color = "#e5e7eb"),
        hoverinfo = "skip",
        showlegend = FALSE
      )
    ...
    p %>%
      plotly_dark(
        xaxis = list(title = list(text = paste0("Average ", value_mode_unit(input$impact_value_mode), " per expected pick"), font = list(size = 15)),
  ...
  pick_movers_table <- function(value_mode, year_filter, round_filter) {
    ...
    empty_tbl <- tibble(
      Direction = character(), Owner = character(), Round = integer(), Pick = character(),
      Current = numeric(), `3-2-1` = numeric(), !!delta_nm := numeric()
    )
    ...
    if (nrow(rows) == 0L) return(empty_tbl)
    ...
    selected <- bind_rows(
      rows %>% arrange(desc(.data$delta)) %>% slice_head(n = 10),
      rows %>% arrange(.data$delta) %>% slice_head(n = 10)
    ) %>%
      distinct(display_asset_id, .keep_all = TRUE)

    selected %>%
      transmute(Direction, Owner = .data$owner, Round = .data$round, Pick, Current, `3-2-1`, !!delta_nm := Delta)
  }
  ...
  render_pick_movers_dt <- function(value_mode) {
    ...
    dt <- datatable(
      ...
      options = list(pageLength = 20, dom = "t", ordering = TRUE,
                     columnDefs = list(list(className = "dt-right", targets = c(4, 5, 6))))
    )
    if (delta_col %in% names(tbl)) {
      dt <- dt %>% formatStyle(delta_col, color = styleInterval(0, c("#ef4444", "#10b981")))
    }
    dt
  }
  ...
  output$tm_teamA_hdr <- renderText(input$tm_teamA %||% "Team 1")
  output$tm_teamB_hdr <- renderText(input$tm_teamB %||% "Team 2")
  ...
  output$tm_picksA_out_hdr <- renderUI(tags$span(style = "font-size:18px; font-weight:850;", sprintf("%s outgoing picks", input$tm_teamA %||% "Team 1")))
  output$tm_picksB_out_hdr <- renderUI(tags$span(style = "font-size:18px; font-weight:850;", sprintf("%s outgoing picks", input$tm_teamB %||% "Team 2")))
  ...
  render_trade_pick_table <- function(ids, side, receiver, pad_to = NULL) {
    ...
    body_rows <- lapply(seq_len(nrow(rows)), function(i) {
      ...
      tags$tr(
        tags$td(style = "min-width:220px; font-size:15px;", tags$strong(r$trade_label)),
    ...
    if (pad_n > 0L) {
      spacer_rows <- lapply(seq_len(pad_n), function(i) {
        tags$tr(class = "tm-spacer-row",
          tags$td(style = "height:34px; padding:6px 0; border-bottom:0;", HTML("&nbsp;")),
          tags$td(style = "height:34px; padding:6px 0; border-bottom:0;", HTML("&nbsp;")),
          tags$td(style = "height:34px; padding:6px 0; border-bottom:0;", HTML("&nbsp;")),
          tags$td(style = "height:34px; padding:6px 0; border-bottom:0;", HTML("&nbsp;"))
        )
  ...
  output$tm_verdict <- renderUI({
    ...
    signed_metric_card <- function(mean_to_A, q05_to_A, q95_to_A, label) {
      ...
      tags$div(style = paste0(
        ...
        tags$div(style = "font-size:30px; line-height:1.1; font-weight:850; color:#10b981; text-align:center;",
                 sprintf("%s %+.1f %s", to_team, shown_mean, label)),
        tags$div(style = "font-size:14px; color:#aaa; margin-top:8px; text-align:center;",
    ...
    best_outcome_card <- function() {
      tags$div(style = sprintf(
        paste0(
          ...
          "flex-direction:column; justify-content:center; height:100%%; min-height:248px; box-sizing:border-box;"
        ), best_col),
        tags$div(style = sprintf("font-size:30px; line-height:1.1; font-weight:850; color:%s; text-align:center;", best_col),
                 sprintf("%s %.1f%% Best Player", best_team, 100 * best_prob)),
        tags$div(style = "font-size:14px; color:#aaa; margin-top:8px; text-align:center;",
                 sprintf("90%% CI: [%.1f%%, %.1f%%]", 100 * best_ci[1], 100 * best_ci[2])),
        tags$div(style = "font-size:12px; color:#777; margin-top:8px; text-align:center;",
                 "Single highest 4-yr WS outcome among selected picks")
      )
    }

    metric_block <- function(metric_card, prob_left, prob_right) {
      tags$div(style = "display:flex; flex-direction:column; gap:8px; min-width:0;",
        metric_card,
        tags$div(style = "display:flex; gap:8px; align-items:stretch; min-width:0;",
          prob_left,
          prob_right
        )
      )
    ...
    tags$div(style = "font-size:13px; color:#bbb; line-height:1.7;",
      tags$div(
        style = "display:grid; grid-template-columns:minmax(0, 2fr) minmax(280px, 1fr); gap:12px; align-items:stretch;",
        tags$div(style = "display:flex; flex-direction:column; gap:12px; min-width:0;",
          metric_block(
