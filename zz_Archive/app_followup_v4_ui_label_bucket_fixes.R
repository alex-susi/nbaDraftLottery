...
protection_ranges_app <- function(protection, round = 1L) {
  protection <- as.character(protection %||% NA_character_)
  if (is.na(protection) || protection %in% c("", "none", "complex")) {
    return(list(keep = NA_character_, convey = NA_character_))
  }
  ...
  list(keep = NA_character_, convey = NA_character_)
}
  ...
  out %>%
    mutate(
      impact_bucket = {
        ...
        short_is_swap_or_cond <- stringr::str_detect(short_txt, "own or|swap|protect|conditional|complex|favorable|ranked|pool|\\bif\\b|otherwise")
        ...
        dplyr::case_when(
          short_is_incoming_outright ~ "Incoming outright picks",
          short_is_swap_or_cond | constrained ~ "Swaps / protections",
          !own_team & simple_outright ~ "Incoming outright picks",
          own_team ~ "Own picks",
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        p(style = "font-size:11px; color:#888; line-height:1.6;",
          ...
          "The x-axis is average value per pick, the y-axis is expected pick count, ",
...
server <- function(input, output, session) {
  ...
  output$impact_chart <- renderPlotly({
    ...
    p %>%
      plotly_dark(
        xaxis = list(title = list(text = paste0("Average ", value_mode_unit(input$impact_value_mode), " per pick"), font = list(size = 15)),
  ...
  output$tm_verdict <- renderUI({
    ...
    signed_metric_card <- function(mean_to_A, q05_to_A, q95_to_A, label) {
      ...
      tags$div(style = paste0(
        ...
        tags$div(style = "font-size:30px; line-height:1.1; font-weight:850; color:#10b981; text-align:left;",
    ...
    best_outcome_card <- function() {
      tags$div(style = sprintf(
        ...
        tags$div(style = sprintf("font-size:30px; line-height:1.1; font-weight:850; color:%s; text-align:left;", best_col),
